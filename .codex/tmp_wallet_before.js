...
const txt = fs.readFileSync('apps/web/.env.local','utf8');
const get=(k)=>{ const line=txt.split(/\\r?\\n/).find(l=>new RegExp('^\\\\s*'+k+'\\\\s*=').test(l)); if(!line) return null; return line.split('=').slice(1).join('=').trim().replace(/^['\"]|['\"]$/g,''); };
