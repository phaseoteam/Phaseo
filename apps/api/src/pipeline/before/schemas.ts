...
const providerSchema = z
    .object({
        ...
        availability: z.object({
            ...
            country_source: z.literal("request_origin").default("request_origin"),
            unknown_country: z.enum(["allow", "deny"]).default("deny"),
            blocked_subdivisions: z.array(z.string().regex(/^[A-Z]{2}-[A-Z0-9]{1,3}$/)).default([]),
            unknown_subdivision: z.enum(["allow", "deny"]).default("allow"),
            reason: z.string().nullable().optional(),
    ...
    .transform<GatewayProviderSnapshot>((provider) => ({
        ...
        availabilityPolicy: provider.availability ? {
            mode: provider.availability.mode,
            countries: provider.availability.countries,
            countrySource: provider.availability.country_source,
            unknownCountry: provider.availability.unknown_country,
            blockedSubdivisions: provider.availability.blocked_subdivisions,
            unknownSubdivision: provider.availability.unknown_subdivision,
            reason: provider.availability.reason ?? null,
            sourceUrl: provider.availability.source_url ?? null,
            effectiveFrom: provider.availability.effective_from ?? null,
