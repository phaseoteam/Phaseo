...
const TAG_PATTERN = /<[^>]+>/g;
const PRICE_TEXT_PATTERN = /(?:pricing|price|\$\s*\d|usd\s*\d|\d\s*usd|per\s*(?:million|1m|mtok)|\/\s*(?:m|mtok)|?|cny|rmb|?)/i;
const NON_CONTENT_PATTERN = /<(?:script|style|svg|noscript|template)\b[^>]*>[\s\S]*?<\/(?:script|style|svg|noscript|template)>/gi;
