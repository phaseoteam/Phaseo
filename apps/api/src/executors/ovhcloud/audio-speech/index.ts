...
export const executor: ProviderExecutor = async args => {
	...
	if (unsupported) return { kind: "completed", upstream: Response.json({ error: { message: `OVHcloud Riva speech does not support this ${unsupported}.` } }, { status: 400 }), bill: { cost_cents: 0, currency: "USD" } };
	...
	const upstream = await (args.upstreamTiming?.fetch ?? fetch)(`https://${model}.endpoints.kepler.ai.cloud.ovh.net/api/v1/tts/text_to_audio`, { method: "POST", headers: { Authorization: `Bearer ${key.key}`, "Content-Type": "application/json", Accept: "application/octet-stream", ...upstreamTestHeaders(args.meta) }, body: JSON.stringify(body) });
	const bill = { cost_cents: 0, currency: "USD", usage: upstream.ok ? { input_characters: ir.input.length, requests: 1 } : undefined };
