...
export const executor: ProviderExecutor = async args => {
	...
	if (unsupported) return { kind: "completed", upstream: Response.json({ error: { message: `OVHcloud SDXL does not support ${unsupported}.` } }, { status: 400 }), bill: { cost_cents: 0, currency: "EUR" } };
	...
	const upstream = await (args.upstreamTiming?.fetch ?? fetch)("https://stable-diffusion-xl.endpoints.kepler.ai.cloud.ovh.net/api/text2image", { method: "POST", headers: { Authorization: `Bearer ${key.key}`, "Content-Type": "application/json", Accept: "application/octet-stream" }, body: JSON.stringify(body) });
	const common = { upstream, keySource: key.source, byokKeyId: key.byokId };
	if (!upstream.ok) return { kind: "completed", ...common, bill: { cost_cents: 0, currency: "EUR" } };
	...
	return { kind: "completed", ...common, ir: response, bill: { cost_cents: 0, currency: "EUR", usage, finish_reason: "stop" } };
};
