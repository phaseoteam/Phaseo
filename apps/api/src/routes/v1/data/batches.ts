...
export function splitGatewayBatchCreatePayload(payload: Record<string, unknown>): {
	...
	const webhookEndpointId =
		typeof payload.webhook_endpoint_id === "string" && payload.webhook_endpoint_id.trim().length > 0
			? { endpoint_id: payload.webhook_endpoint_id.trim() }
			: null;
	const webhook =
		rawWebhook && typeof rawWebhook === "object" && !Array.isArray(rawWebhook)
			? (rawWebhook as Record<string, unknown>)
			: webhookEndpointId;
	const invalidWebhook = hasWebhook && rawWebhook != null && !webhook;
	return {
		upstreamPayload,
		webhook,
		...(webhookEndpointId && !hasWebhook ? {} : { invalidWebhook }),
	};
}
...
async function handleCreate(req: Request) {
	...
	const inputFileId = toText(upstreamPayload.input_file_id) ?? toText(payload.input_file_id);
