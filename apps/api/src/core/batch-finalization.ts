...
function extractCustomId(entry: any, fallbackIndex?: number): string | null {
	return normalizeText(entry?.custom_id ?? entry?.customId ?? entry?.id ?? entry?.request?.custom_id)
		?? (typeof fallbackIndex === "number" ? `response-${fallbackIndex + 1}` : null);
}
...
export async function finalizeBatchJob(args: FinalizeBatchJobArgs): Promise<FinalizeBatchJobResult> {
	...
	if (record.meta?.outputFileId && settlement.outputEntries) {
		const rows = settlement.outputEntries.map((entry, index): BatchRequestRowInput => {
			...
			return {
				...
				customId: extractCustomId(entry, index) ?? `response-${index + 1}`,
				requestIndex: index,
