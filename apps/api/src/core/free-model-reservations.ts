...
export type FreeModelReservationIdentity = z.infer<typeof Identity>;
 ...
 */
export async function reserveFreeModelOverage(input: FreeModelReservationIdentity) {
    ...
    const result = await reserveWalletCredits({ workspaceId: args.workspaceId, keyId: args.keyId,
        ...
        amountNanos: FREE_MODEL_OVERAGE_NANOS, requestCount: 1 });
 ...
 */
export async function finalizeFreeModelOverage(input: FreeModelReservationIdentity, outcome: "capture" | "release") {
    const args = reservation(input);
    ...
    const result = outcome === "capture"
        ? await captureWalletReservation({ workspaceId: args.workspaceId, keyId: args.keyId,
            reservationId: args.reservationId, captureRefId: args.requestId })
        : await releaseWalletReservation({ workspaceId: args.workspaceId, keyId: args.keyId,
            reservationId: args.reservationId, releaseRefId: args.requestId });
