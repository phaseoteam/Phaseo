 ...
 */
export function irToOpenAIResponses(ir: IRChatRequest, providerModelSlug?: string | null, providerId?: string): any {
	...
	for (const msg of ir.messages) {
		if (msg.role === "system") {
			...
			inputItems.push({
				type: "message",
				role: "assistant",
				content: msg.content.map(mapAssistantContentPart),
			});
