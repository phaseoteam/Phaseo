...
type AppsUsageListProps = {
	dataByRange: Partial<Record<AppsUsageRange, TopAppData[]>>;
	defaultRange?: AppsUsageRange;
	showHeader?: boolean;
	title?: string;
	icon?: ReactNode;
	maxCollapsed?: number;
...
type AppsUsageRange = "today" | "week" | "month";

const RANGE_OPTIONS: Array<{ key: AppsUsageRange; label: string }> = [
...
export function AppsUsageList({
	...
	const [range, setRange] = useState<AppsUsageRange>(() => {
			...
			{showHeader ? (
				<div className="flex flex-wrap items-center justify-between gap-3">
					...
					<Select
						value={range}
						onValueChange={(value) => {
							setRange(value as AppsUsageRange);
							setShowAll(false);
						}}
					>
