...
function fallbackLabel(modelId: string): string {
	...
	return titleCase(
		leaf
			.replace(/\b(\d{4}-\d{2}-\d{2}|preview|mini|nano|turbo|latest)\b/gi, "")
			.replace(/-{2,}/g, "-")
			.replace(/^-|-$/g, "")
	);
}
...
export function buildHomeGatewayModelOptions(
	models: GatewaySupportedModel[]
): HomeGatewayModelOption[] {
	const map = new Map<
		string,
		{
			id: string;
	...
	for (const model of models) {
		...
		const provider =
			model.organisationName ?? model.providerName ?? titleCase(logoId);
	...
	const options = Array.from(map.values()).map((entry) => {
		...
		const providerSummary =
			providerCount > 1 ? `${providerCount} gateway providers` : entry.provider;
