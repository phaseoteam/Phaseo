				...
				<div className="grid grid-cols-1 gap-3">
					{Array.from({ length: CARD_LIMIT }).map((_, index) => (
						<div
							...
							className="h-20 animate-pulse rounded-[20px] bg-zinc-100 dark:bg-zinc-800"
						/>
					))}
				</div>
			</div>
				...
				<div className="grid grid-cols-1 gap-3">
					{modelCards.map((card) => (
						<UpdateCard
							...
							className={cn(
								"group",
								"rounded-[20px] border border-zinc-200 bg-white/90 py-0 shadow-sm ring-1 ring-inset ring-zinc-200/60",
