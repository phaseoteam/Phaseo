...
export function ArtificialAnalysisBenchmarks({ highlights, results = [], rankings = [], modelId, modelName, initialExpandedMetric = null }: {
	...
	const comparisonEntries = (benchmarkId: string) => {
		...
		const current: PublicBenchmarkRankingEntry = {
			...
			score: (ranking?.lower_is_better ? Math.min : Math.max)(...configurations.map((item) => item.score)),
			rank: existing?.rank ?? 0,
			configurations,
		};
		return [...entries.filter((entry) => entry.model_id !== modelId), current].sort((left, right) => (ranking?.lower_is_better ? left.score - right.score : right.score - left.score) || left.model_name.localeCompare(right.model_name));
	};
	...
	return <section aria-label="Artificial Analysis benchmarks">
		...
		<div className="mt-4 grid grid-cols-2 border-t lg:grid-cols-4">
			{artificialAnalysisMetrics.map(({ id: fallbackId, key, label }, index) => {
				...
				return <button key={id} type="button" aria-expanded={active} onClick={() => { setPickerOpen(false); setExpandedMetric(active ? null : id); }} className={`group min-w-0 py-3 text-left sm:py-4 ${index % 2 ? "border-l pl-4 sm:pl-5" : "pr-4 sm:pr-5"} ${index > 1 ? "border-t lg:border-t-0 lg:border-l lg:pl-5" : ""}`}>
					...
					<span className="mt-2 flex flex-wrap items-baseline gap-x-1.5 gap-y-1 text-xl font-semibold tracking-tight sm:text-2xl"><span>{result?.score == null ? "—" : <AnimatedScore benchmarkId={id} score={result.score} />}</span>{result?.score != null && rank ? <span className="text-xs font-medium text-muted-foreground" title={`Rank ${rank.rank} of ${rank.total} evaluated configurations`}>#{rank.rank} of {rank.total} configurations</span> : null}</span>
				</button>;
			})}
		</div>
		...
		{expandedMetric ? <div className="border-b py-5" aria-live="polite">
			<div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
				<div><h3 className="text-sm font-medium">{activeMetric?.label} comparison</h3><p className="text-xs text-muted-foreground">{activeRanking?.lower_is_better ? "Lower is better" : "Higher is better"} · ranked across evaluated configurations</p></div>
