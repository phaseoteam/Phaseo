...
export function ArtificialAnalysisBenchmarks({ highlights, results = [], rankings = [], modelId }: {
	...
	return <section aria-label="Artificial Analysis benchmarks">
		...
		{expandedMetric ? <div className="border-b py-5" aria-live="polite">
			...
			{chartRows.length ? <ScrollArea className="w-full" scrollBarOrientation="horizontal" keepScrollbarMounted viewportClassName="pb-3"><div className="relative h-[390px] border-b" style={{ width: `${Math.max(chartRows.length * 64 + 16, 360)}px`, minWidth: "100%" }}>
				...
				<div className="absolute inset-x-2 bottom-0 top-4 flex items-end gap-1.5">{chartRows.map(({ entry, configuration }) => {
					const height = Math.max((Number(configuration.score) / maxScore) * 250, 5);
					const colour = safeColour(entry.organisation_colour, entry.model_id === modelId ? purple : "#6b7280");
					return <Tooltip key={entry.model_id}><TooltipTrigger asChild><div className="group flex h-full w-14 shrink-0 flex-col items-center justify-end"><span className="mb-1 text-[11px] font-semibold tabular-nums">{expandedMetric === "aa-intelligence-index-cost-v4" ? `$${Number(configuration.score).toLocaleString(undefined, { maximumFractionDigits: 2 })}` : Number(configuration.score).toFixed(1)}</span><span className="w-10 rounded-t-[3px] transition-[height,filter] duration-300 group-hover:brightness-110" style={{ height, backgroundColor: colour }} /><div className="relative h-[112px] w-full"><span className="absolute left-1/2 top-2 w-24 origin-top-left -rotate-[55deg] text-left text-[11px] leading-[1.1]">{entry.model_name} <span className="text-muted-foreground">({configurationLabel(configuration.variant)})</span></span></div></div></TooltipTrigger><TooltipContent side="top" className="max-w-64"><p className="font-medium">{entry.model_name}</p><p className="text-muted-foreground">{entry.organisation_name ?? "Unknown organization"} · {configurationLabel(configuration.variant)}</p><p className="mt-1 tabular-nums">{activeMetric?.label}: {expandedMetric === "aa-intelligence-index-cost-v4" ? `$${Number(configuration.score).toLocaleString(undefined, { maximumFractionDigits: 2 })}` : Number(configuration.score).toFixed(1)} · rank #{entry.rank} of {entries.length}</p></TooltipContent></Tooltip>;
				})}</div>
