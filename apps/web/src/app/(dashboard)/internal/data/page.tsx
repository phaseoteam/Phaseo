		...
		<main className="container mx-auto max-w-7xl space-y-8 py-8 lg:py-10">
			...
			<div className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.55fr)]">
				...
				<aside className="space-y-4">
					...
					<div className="rounded-xl border bg-card p-5"><div className="flex items-center gap-2"><CircleAlert className="size-4" /><h2 className="font-medium">Needs attention</h2></div><div className="mt-4 space-y-3 text-sm"><Link href="/internal/data/models?attention=hidden" className="flex items-center justify-between rounded-md border px-3 py-2.5 hover:bg-muted/40"><span>Hidden models</span><Badge variant={overview.attention.hiddenModels ? "secondary" : "outline"}>{overview.attention.hiddenModels}</Badge></Link><Link href="/internal/data/models?attention=missing-organisation" className="flex items-center justify-between rounded-md border px-3 py-2.5 hover:bg-muted/40"><span>Missing organisation</span><Badge variant={overview.attention.modelsWithoutLab ? "destructive" : "outline"}>{overview.attention.modelsWithoutLab}</Badge></Link></div></div>
					<div className="rounded-xl border border-dashed p-5"><div className="flex items-center gap-2"><Landmark className="size-4 text-muted-foreground" /><h2 className="text-sm font-medium">Publishing</h2></div><p className="mt-2 text-xs leading-5 text-muted-foreground">The database is authoritative. JSON snapshots are scheduled daily at 00:00 UTC for review and recovery.</p></div>
				</aside>
			</div>
		</main>
	);
