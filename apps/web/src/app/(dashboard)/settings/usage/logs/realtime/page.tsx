...
async function SessionList({ searchParams }: { searchParams: Promise<{ page?: string; per_page?: string }> }) {
	...
	const requestedSize = Number(params.per_page);
	const pageSize = [25, 50, 100].includes(requestedSize) ? requestedSize : 50;
	let data: { pageSize?: number; sessions: RealtimeSession[]; hasMore: boolean };
	try {
		data = await fetchAccountWebApi(`/api/account/settings/usage/realtime?workspaceId=${encodeURIComponent(context.workspaceId)}&page=${page}&pageSize=${pageSize}`, context.accessToken);
	} catch {
		return <p role="alert">Realtime sessions could not be loaded. Refresh to try again.</p>;
	}
	return <RealtimeSessionsPanel key={context.workspaceId} sessions={data.sessions} page={page} pageSize={data.pageSize ?? 50} hasMore={data.hasMore && page < 10000} />;
}
