...
import type { SettingsResourceData } from "../../cachedSettingsActions";
...
export default function PresetFeedbackClient() {
	const params = useSearchParams();
	const parsedFilters = parseFilters(Object.fromEntries(params));
	const { fromIso, toIso, rating, metadataKey, metadataValue } = parsedFilters;
	const parameters = JSON.stringify({ fromIso, toIso, rating, metadataKey, metadataValue });
	return <PrivateSettingsQuery<SettingsResourceData<"preset-feedback">> path="/api/account/settings/preset-feedback" resource="preset-feedback" parameters={parameters}>{(data) => <PresetFeedbackContent data={data} parsedFilters={parsedFilters} />}</PrivateSettingsQuery>;
}
