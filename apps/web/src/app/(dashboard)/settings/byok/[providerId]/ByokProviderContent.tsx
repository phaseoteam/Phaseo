...
import type { SettingsByokInitialData, SettingsKeysInitialData } from "@/lib/fetchers/internal/settingsTypes";
...
import type { fetchFrontendAPIProviderHeader, fetchFrontendAPIProviderModels } from "@/lib/fetchers/frontend/fetchPublicCatalog";
type Props = { providerId: string; sampleKeys?: string; catalogProvider: Awaited<ReturnType<typeof fetchFrontendAPIProviderHeader>>; providerModels: Awaited<ReturnType<typeof fetchFrontendAPIProviderModels>> };
...
export default function ByokProviderContent(props: Props) {
	return <SettingsResourceQuery resource="byok">{(initialData) => <SettingsResourceQuery resource="keys">{(keysData) => <ByokProviderPage {...props} initialData={initialData} keysData={keysData} />}</SettingsResourceQuery>}</SettingsResourceQuery>;
}

function ByokProviderPage({ providerId, sampleKeys, catalogProvider, providerModels, initialData, keysData }: Props & { initialData: SettingsByokInitialData; keysData: SettingsKeysInitialData }) {
