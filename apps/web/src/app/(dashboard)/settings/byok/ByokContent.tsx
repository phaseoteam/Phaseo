...
import type { fetchFrontendAPIProviders } from "@/lib/fetchers/frontend/fetchPublicCatalog";
import { MAX_BYOK_KEYS_PER_PROVIDER } from "@/lib/byok/constants";

type Providers = Awaited<ReturnType<typeof fetchFrontendAPIProviders>>;
...
export default function BYOKPage({ providerCatalogData }: { providerCatalogData: Providers }) {
	return (
		<div className="mx-auto space-y-6">
			...
			<SettingsResourceQuery resource="byok">{(initialData) => <ByokProvidersSection initialData={initialData} providerCatalogData={providerCatalogData} />}</SettingsResourceQuery>
		</div>
	);
}

function ByokProvidersSection({ initialData, providerCatalogData }: { initialData: SettingsByokInitialData; providerCatalogData: Providers }) {
