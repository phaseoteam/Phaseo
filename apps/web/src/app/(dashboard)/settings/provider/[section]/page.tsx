...
export default function ProviderSettingsPage({ params }: { params: Promise<{ section: string }> }) {
	return <Suspense fallback={<SettingsSectionFallback />}><ProviderSettingsContent params={params} /></Suspense>;
}

async function ProviderSettingsContent({ params }: { params: Promise<{ section: string }> }) {
	const { section } = await params;
	if (!["models", "review", "integrations"].includes(section)) notFound();
	const data = await fetchSettingsProviderOnboardingInitialData();
