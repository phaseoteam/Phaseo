...
`phaseo.app/api/chat/*`, and `phaseo.app/api/internal/*`. Vercel retains all
authentication and Stripe/checkout routes, including
`/api/webhooks/stripe-checkout`. The external gateway at `api.phaseo.app`
remains a separate Worker in `apps/api`.
