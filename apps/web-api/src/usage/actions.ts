...
import { metadataForIds } from "@/routes/account/settings-usage";
import {
...
const usageStorage = new AsyncLocalStorage<UsageExecutionContext>();
const current = () => { const value = usageStorage.getStore(); if (!value) throw new Error("usage_context_missing"); return value; };
export function runWithUsageContext<T>(context: UsageExecutionContext, callback: () => Promise<T>) { return usageStorage.run(context, callback); }
const cache = <T extends (...args: any[]) => any>(fn: T): T => fn;
async function createClient(): Promise<SupabaseClient> { return current().account.client; }
function createAdminClient(): SupabaseClient { return current().account.client; }
async function getWorkspaceIdFromCookie() { return current().account.workspaceId; }
async function requireAuthenticatedUser() { return { supabase: current().account.client, user: current().account.user }; }
...
async function requireAuthedTeamContext(
	...
	void supabase;
	return { user: current().account.user, workspaceId: await getWorkspaceIdFromCookie() };
}
...
async function fetchUsageMetadata(args: { models?: string[]; providers?: string[]; apps?: string[] }): Promise<{
	...
	return await metadataForIds(current().account, args) as any;
}
...
const fetchOrganizationColorsCached = cache(async (
	...
	const entries = (await fetchUsageMetadata({ models: modelIds })).modelMetadataEntries;
	const colors = new Map<string, string>();
	for (const [id, metadata] of entries) {
		const colour = metadata.organisationColour;
		if (!colour) continue;
		colors.set(id, colour);
		if (metadata.organisationId) {
			colors.set(metadata.organisationId, colour);
			colors.set(metadata.organisationId.toLowerCase(), colour);
		}
		const withoutOrg = id.includes("/") ? id.split("/").slice(1).join("/") : null;
		if (withoutOrg) colors.set(withoutOrg, colour);
		if (id.includes(":")) colors.set(id.split(":")[0], colour);
	}
	return colors;
});
...
> => {
	...
	const metadataDebugEnabled =
		current().env.USAGE_MODEL_METADATA_DEBUG === "1" ||
		current().env.NEXT_PUBLIC_USAGE_MODEL_METADATA_DEBUG === "1";
...
const fetchProviderNamesCached = cache(async (
	...
	return new Map((await fetchUsageMetadata({ providers: providerIds })).providerNameEntries);
});
...
const fetchProviderMetadataCached = cache(async (
	...
	const providerIds = providerIdsKey ? providerIdsKey.split("\n") : [];
	if (providerIds.length === 0) return new Map();
	return new Map((await fetchUsageMetadata({ providers: providerIds })).providerMetadataEntries);
});
...
const fetchAppNamesCached = cache(async (
	...
	return new Map((await fetchUsageMetadata({ apps: appIds })).appNameEntries);
});
...
export async function fetchChartData(
	...
	if (params.forceLive) {
		...
		} else if (rawFallbackEnabled() && currentRows.length === 0) {
	...
	if (rawFallbackEnabled() && previousRows.length === 0) {
...
const fetchAppMetadataCached = cache(async (
	...
	const appIds = appIdsKey ? appIdsKey.split("\n") : [];
	if (appIds.length === 0) return new Map();
	return new Map((await fetchUsageMetadata({ apps: appIds })).appMetadataEntries);
});
