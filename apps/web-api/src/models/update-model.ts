...
const MODEL_MODALITY_OPTIONS = ["text", "image", "audio", "audio_stt", "audio_tts", "audio_music", "video", "embedding", "rerank", "moderation"] as const
type CapabilityStatusOption = "active" | "deranked_lvl1" | "deranked_lvl2" | "deranked_lvl3" | "disabled"
function normalizeModelStatus(value: string | null | undefined) {
  const normalized = (value ?? "").trim().toLowerCase()
  if (normalized === "rumoured") return "Rumoured"
  if (normalized === "announced") return "Announced"
  if (["limited access", "limited_access", "limited-access", "private preview", "private_preview", "restricted"].includes(normalized)) return "Limited Access"
  if (normalized === "withheld") return "Withheld"
  if (normalized === "deprecated") return "Deprecated"
  if (normalized === "retired") return "Retired"
  return "Released"
}
function normalizeCapabilityStatus(value: string | null | undefined): CapabilityStatusOption {
  const normalized = (value ?? "").trim().toLowerCase()
  if (normalized === "disabled") return "disabled"
  if (["deranked_lvl1", "deranked_lvl_1", "deranked"].includes(normalized)) return "deranked_lvl1"
  if (["deranked_lvl2", "deranked_lvl_2"].includes(normalized)) return "deranked_lvl2"
  if (["deranked_lvl3", "deranked_lvl_3"].includes(normalized)) return "deranked_lvl3"
  return "active"
}
...
export async function updateModelData(supabase: SupabaseClient, payload: ModelUpdatePayload) {
...
export async function deleteBenchmarkResultData(supabase: SupabaseClient, id: string) {
  ...
  return { ok: true, modelId: benchmarkRow?.model_id ?? null, benchmarkId: benchmarkRow?.benchmark_id ?? null }
}

export async function deletePricingRuleData(supabase: SupabaseClient, id: string) {
...
export async function deleteProviderModelData(supabase: SupabaseClient, id: string) {
  ...
  return { ok: true, modelId: providerModelRow?.model_id ?? null }
}

export async function deleteModelGraphData(supabase: SupabaseClient, modelId: string) {
  const providerRows = await supabase.from("data_api_provider_models").select("provider_api_model_id,provider_id,api_model_id").eq("model_id", modelId)
  if (providerRows.error) throw providerRows.error
  const providerIds = (providerRows.data ?? []).map((row) => row.provider_api_model_id).filter(Boolean)
  if (providerIds.length) {
    const capabilities = await supabase.from("data_api_provider_model_capabilities").delete().in("provider_api_model_id", providerIds)
    if (capabilities.error) throw capabilities.error
  }
  await deletePricingRulesForProviderPairs(supabase, (providerRows.data ?? []).map((row) => ({ provider_id: row.provider_id, api_model_id: row.api_model_id })))
  for (const [table, column] of [
    ["data_api_provider_models", "model_id"],
    ["data_api_model_aliases", "api_model_id"],
    ["data_benchmark_results", "model_id"],
    ["data_model_details", "model_id"],
    ["data_model_links", "model_id"],
    ["data_subscription_plan_models", "model_id"],
    ["data_models", "model_id"],
    ["data_api_models", "api_model_id"],
  ] as const) {
    const result = await supabase.from(table).delete().eq(column, modelId)
