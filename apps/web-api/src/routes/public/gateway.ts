...
async function gatewayModels(env: Env) {
	...
	for (let offset = 0; offset < providerModelIds.length; offset += 200) { const result = await client.from("data_api_provider_model_capabilities").select("provider_api_model_id,capability_id,params,status").in("provider_api_model_id", providerModelIds.slice(offset, offset + 200)); if (result.error) throw result.error; for (const row of result.data ?? []) { if (!row.provider_api_model_id || !row.capability_id || ["disabled", "internal_testing"].includes(String(row.status ?? "").toLowerCase())) continue; const values = capabilities.get(row.provider_api_model_id) ?? new Set<string>(); values.add(row.capability_id); capabilities.set(row.provider_api_model_id, values); const params = capabilityParams.get(row.provider_api_model_id) ?? {}; params[String(row.capability_id)] = row.params && typeof row.params === "object" ? row.params : {}; capabilityParams.set(row.provider_api_model_id, params); } }
