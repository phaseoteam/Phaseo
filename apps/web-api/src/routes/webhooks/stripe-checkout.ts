...
import { activeStripeCustomer, getStripe } from "@/billing/stripe";
...
async function resolveCustomerIdentity(
    ...
    try {
        ...
        const activeCustomer = activeStripeCustomer(customer);
        if (!activeCustomer) {
            return { email: null, firstName: "" };
        }
        const email =
            typeof activeCustomer.email === "string" && activeCustomer.email.trim().length > 0
            ? activeCustomer.email
            : null;
        const firstName = deriveFirstName(activeCustomer.name ?? email);
...
async function resolveWalletForTopUpPaymentIntent(args: {
    ...
    async function resolveByWorkspaceId(workspaceId: string): Promise<WalletAttributionRow | null> {
        ...
        const byWorkspaceWallet = (byWorkspaceWalletRaw ?? null) as unknown as WalletAttributionRow | null;
    ...
    return (byCustomerWalletRaw ?? null) as unknown as WalletAttributionRow | null;
}
...
stripeWebhookRouter.use("*", async (c, next) => {
    await next();
    for (const [name, value] of Object.entries(PRIVATE_NO_STORE_HEADERS)) c.header(name, value);
});

stripeWebhookRouter.post("/stripe-checkout", async (c) => {
    const req = c.req.raw;
    ...
    try {
        ...
        const configuredSecret = c.env.STRIPE_WEBHOOK_SECRET ?? "";
        ...
        switch (event.type) {
            ...
            case "payment_intent.succeeded": {
                ...
                if (customerIdentity.email) {
                    try {
                        await sendCreditsPurchasedEvent(c.env, {
                ...
                try {
                    await sendBillingDiscordWebhook(c.env, {
