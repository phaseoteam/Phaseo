...
## AI Stats Gateway is now the Conduit

The AI Stats Gateway has been renamed and repositioned as the **AI Stats Conduit** — a single API for every model. Conduit emphasizes unified routing, provider portability, and instant access to new modalities without rewriting integrations. This is the same trusted infrastructure, now focused as the definitive backbone for model access, reliability, and observability at scale.
...
-   Introduced a Playground for easily testing models through the gateway in a simple but powerful interface
