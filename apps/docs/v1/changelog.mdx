---
icon: "newspaper"
title: "September"
description: "September 2026 model releases across Phaseo."
rss: true
---

<Update label="September 10, 2026" tags={["Model Releases", "Pricing"]}>

### Model Releases

![DeepSeek V4.1 Flash model release artwork](../images/model-releases/2026-09-10--deepseek--deepseek-v4.1-flash.png)

- [DeepSeek V4.1 Flash](https://phaseo.app/models/deepseek/deepseek-v4.1-flash)
- DeepSeek Pro requests will follow V4.1 Flash after the temporary Preview route expires on September 10.
- Flash-series pricing changes at 12:00 Beijing time: $0.003 per million cached input tokens, $0.15 per million uncached input tokens, and $0.60 per million output tokens off-peak; peak-hour rates are double.

</Update>

<Update label="September 3, 2026" tags={["Model Releases"]}>

### Model Releases

![GPT-6 Astra model release artwork](../images/model-releases/2026-09-03--openai--gpt-6-astra.png)

- [GPT-6 Astra](https://phaseo.app/models/openai/gpt-6-astra)

- [Ling 3.0 Flash Sante](https://phaseo.app/models/inclusionai/ling-3.0-flash-sante)
- [Ling 3.0 Flash VL](https://phaseo.app/models/inclusionai/ling-3.0-flash-vl)

</Update>

<Update label="September 2, 2026" tags={["Model Releases"]}>

### Model Releases

![Gemini 3.8 Flash model release artwork](../images/model-releases/2026-09-02--google--gemini-3.8-flash.png)

- [Gemini 3.8 Flash](https://phaseo.app/models/google/gemini-3.8-flash)
- [Muse Spark 1.3](https://phaseo.app/models/meta/muse-spark-1.3)
- [Muse Spark 1.3 Contributor](https://phaseo.app/models/meta/muse-spark-1.3-contributor)
- [Qwen 3.8 Max 0902](https://phaseo.app/models/qwen/qwen3.8-max-0902)

</Update>

<Update label="September 1, 2026" tags={["Model Releases"]}>

### Model Releases

![Claude Fable 5.1 model release artwork](../images/model-releases/2026-09-01--anthropic--claude-fable-5.1.png)

- [Claude Fable 5.1](https://phaseo.app/models/anthropic/claude-fable-5.1)
- [Muse Voice Transcribe 1.0](https://phaseo.app/models/meta/muse-voice-transcribe-1.0)

</Update>
