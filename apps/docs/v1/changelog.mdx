...
rss: true
---
<Update label="16th May 2026">
# Provider Activation, Regions, and Free Router Fixes

We tightened model-routing visibility across the product so it is clearer which providers are active, where they can execute, and how free-router and pricing views should behave.

This release includes:

- Active gateway provider states are now surfaced more consistently across model pages, comparison views, and model discovery flows.
- The models dashboard now supports execution-region filtering, making it easier to narrow results to EU, US, APAC, Japan, and Australia coverage where available.
- Free Router overview and model detail flows now resolve more reliably, including the dedicated free-router model page and related monitor fetches.
- Homepage gateway token and monthly request summaries now use the corrected public metrics fetch path instead of under-reporting current activity.

</Update>

<Update label="7th May 2026">
# Clearer Provider Availability and Pricing

We refreshed model pages and routing visibility so it is easier to see which providers are currently usable for a model and how pricing compares at a glance.

This release includes:

- Provider activation states are now exposed across catalog and quickstart surfaces, so availability is clearer before you send a request.
- Models pages now have better pricing summaries, cleaner hover cards, and improved minute-price display for realtime usage.
- OpenAI realtime model coverage has been refreshed across product and gateway surfaces.
- Recent catalog fixes also improve model family naming consistency and reduce missing-model edge cases.

</Update>

<Update label="6th May 2026">
# Video and Async Jobs Rollout

We expanded AI Stats video support and made async activity easier to inspect from the gateway.

This release includes:

- Broader video lifecycle coverage, including better status, cancellation, content, and download flows.
- More complete batch lifecycle handling across API and gateway tooling.
- Richer usage observability with sessions, async jobs, request details, and better filtering in usage logs.
- Improved replay and debugging detail for gateway requests so failures are easier to understand and retry.

</Update>

<Update label="25th April 2026">
# Low-Balance Alerts and Billing Safeguards

We added low-balance alerting for gateway workspaces and tightened several pricing and billing edge cases.

This release includes:

- Low-balance email alert wiring for supported workspace billing flows.
- Resend onboarding inside settings for teams that want alert destinations configured in-product.
- Safer pricing behavior for non-standard tiers and clearer unreleased-model banner handling.
- Follow-up fixes across subscription plan sync, usage rollups, and workspace billing flows.

</Update>

<Update label="21st April 2026">
# Announcements Hub and Workspace Management

We introduced a dedicated announcements hub and expanded workspace management across settings.

This release includes:

- A new announcements area on the site and homepage so product and incident updates are easier to follow.
- Workspace management pages for members and settings, with broader navigation cleanup around the workspace model.
- Hardening for invites, membership handling, and related settings flows.
- Follow-up polish for security notices, homepage announcements UX, and workspace naming across the product.

</Update>

<Update label="4th April 2026">
# Mobile Model Pages and Pricing Improvements

This release improves model browsing and pricing clarity on mobile, while keeping billing simpler and more predictable.

This release includes:

- Better mobile behavior for model pages, including cleaner handling of "Other models" cards and long model names.
- Improved mobile pricing layouts to reduce horizontal scrolling and make provider pricing easier to compare.
- More accurate tiered provider visibility for free model routes.
- Updated Pay As You Go top-up fee from 7% to 5%, while keeping the existing $1 minimum fee.

</Update>

<Update label="22nd March 2026">
# Chat Rooms Go Multimodal, Foundry Introduced, and LLM Council Launches

## New language SDKs introduced

We have introduced new language SDKs across the platform to expand access and improve developer choice across ecosystems.

## TypeScript and Python SDKs have been significantly updated

Both the **TypeScript** and **Python** SDKs have received a much-needed update, bringing them in line with recent REST API changes and introducing new features to improve day-to-day integration workflows.

These updates are focused on improving consistency, reducing API surface mismatches, and making SDK behavior more predictable as the REST API evolves.

If you experience any bugs or unexpected behavior, please report them so we can fix them quickly.

## Chat rooms are now multimodal

Chat rooms now support multimodal workflows so you can operate across text and additional modalities in a single conversation flow, instead of splitting work across separate tools.

Right now, you can try **Audio**, **Moderations**, and **Embeddings** models directly in the chatroom.

**Images** and **Video** support are coming soon.

Our goal is to build the best possible experience for you to try and test all of the models we offer.

## Foundry is now available

We are introducing **Foundry** as the place for experimental products and projects inside AI Stats.

Foundry gives us a dedicated space to launch new ideas quickly, gather feedback, and iterate in public before broader rollout across the rest of the platform.

## LLM Council is the first Foundry project

The first Foundry project is **LLM Council**.

LLM Council lets you combine responses from multiple LLMs and synthesize them into what should be a stronger final answer than a single-model response on its own.

LLM Council is available in chat now, with API availability coming very soon.

</Update>

<Update label="12th March 2026">
# Models Display Page Overhaul

We overhauled the **models display page** to make it clearer which models are available and what information is available for each model on the gateway.

This update includes:

- clearer model availability visibility across gateway-supported routes
- cleaner information hierarchy for model details and metadata
- improved readability for model capability and support context

</Update>

<Update label="10th March 2026">
# Brand Refresh

We have rolled out a broader **brand refresh** across AI Stats today.

This release includes:

- refreshed branding across key product surfaces
- updated visual assets and wordmarks
- consistency improvements across the website and documentation

The goal of this refresh is to make AI Stats feel cleaner, more consistent, and more recognisable as we continue to expand the product and gateway.
...
We rolled out **Alibaba Cloud** across AI Stats today, adding another provider option for supported gateway and API workflows.

### [Alibaba Cloud](https://www.alibabacloud.com/help/en/model-studio/getting-started/what-is-model-studio)
Alibaba Cloud gives us another production provider route, including support for its DashScope-compatible model surfaces and broader provider redundancy for teams deploying globally.

This rollout includes:

- provider availability across AI Stats
- gateway coverage for Alibaba Cloud compatible routes
- documentation and provider-surface updates for the new integration

You can explore Alibaba Cloud support through the latest provider coverage and gateway updates.

![Alibaba Cloud](../images/Alibaba-Cloud.png)

</Update>

<Update label="5th March 2026">
# GPT 5.4 and GPT 5.4 Pro Introduced

We introduced **GPT 5.4** and **GPT 5.4 Pro** across AI Stats and exposed both models through the API.

![GPT 5.4](../images/gpt-5-4.png)

This rollout includes:

- Model discovery and availability updates in AI Stats
- Gateway and API availability for GPT 5.4 and GPT 5.4 Pro
- Documentation updates to reflect the latest OpenAI model support
...
We introduced **Gemini 3.1 Flash Lite Preview** across AI Stats and rolled out support across product surfaces.

![Gemini 3.1 Flash Lite Preview](../images/gemini-3.1-flash-lite.png)
...
We have introduced **Nano Banana 2** across AI Stats, released as **Gemini 3.1 Flash Image Preview**.
