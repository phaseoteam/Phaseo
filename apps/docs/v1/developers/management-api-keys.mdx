...
You can create a management API key through the dashboard.
...
### Using the API (Coming Soon)

<Note>
`POST /v1/management/keys` is planned but not generally available yet.
</Note>
...
```bash
curl https://api.phaseo.app/v1/credits?team_id=your-team-id \
...
```bash
curl "https://api.phaseo.app/v1/activity?team_id=your-team-id&days=30" \
  -H "Authorization: Bearer <management-api-key>"
```
...
```bash
curl "https://api.phaseo.app/v1/management/keys?team_id=your-team-id" \
  -H "Authorization: Bearer <management-api-key>"
```
...
```bash
export MANAGEMENT_API_KEY="aistats_v1_sk_xx_xxx"
```
...
# Create support for automated key creation when the endpoint becomes available
RESPONSE=$(curl -s -X POST https://api.phaseo.app/v1/management/keys \
