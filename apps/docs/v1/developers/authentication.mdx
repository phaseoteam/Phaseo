...
Create a key in the [dashboard](https://phaseo.app/gateway/keys), store it on your server, and send it as a bearer token with Gateway requests.
...
Rotate a key by updating your application to use its replacement before invalidating the old credential. See the [key rotation endpoint](../api-reference/endpoint/keys-rotate.mdx).
...
- **429 Too Many Requests**: the key or workspace exceeded a limit. See [Rate limits](../api-reference/limits.mdx).
