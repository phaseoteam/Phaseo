...
description: "Replace Vercel AI Gateway routing with Phaseo Gateway while preserving AI SDK or OpenAI-compatible application behavior."
---
...
- `PHASEO_API_KEY` available in local, staging, and production environments.
...
- Move durable routing defaults and provider restrictions into [Presets](../guides/presets) or [Routing and Fallbacks](../guides/routing-and-fallbacks) instead of rebuilding them ad hoc in every model factory.
...
- `PHASEO_API_KEY` configured in every runtime that previously used the Vercel gateway key.
- Official `@phaseo/ai-sdk-provider` wired in if your app uses the Vercel AI SDK directly.
