...
```bash
phaseo login
phaseo codex --model openai/gpt-5.6-terra
```

Install Codex from its official release first. Setup creates a non-expiring `Phaseo CLI: Codex API Key`, stores it in the OS credential store, and writes a dedicated Phaseo profile. Codex retrieves the key through the CLI helper, so no plaintext key is written to `config.toml`.
...
The same Phaseo CLI workflow also launches [Cline](./cline), [Kilo Code](./kilo-code), [oh-my-pi](./oh-my-pi), [Muse Code](./muse-code), and [MiniMax Code](./minimax-code) with isolated temporary provider configuration.
