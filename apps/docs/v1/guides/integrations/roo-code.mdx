---
title: "Roo Code"
icon: "/icons/brands/roo-code.svg"
description: "Connect Roo Code to Phaseo through an OpenAI-compatible profile."
---

Use the Phaseo CLI to create a dedicated key, then configure Roo Code's provider profile manually. Roo Code keeps credentials in VS Code secret storage, so the final entry happens in the extension UI.

## Set up with the Phaseo CLI

```bash
phaseo login
phaseo roo-code --model anthropic/claude-sonnet-4.6
```

## Configure Roo Code manually
