...
The CLI can launch Cline, Kilo Code, oh-my-pi, Muse Code, and MiniMax Code with temporary provider configuration and a dedicated Phaseo key. Each harness has its own integration guide with setup and troubleshooting details: [Cline](./cline), [Kilo Code](./kilo-code), [oh-my-pi](./oh-my-pi), [Muse Code](./muse-code), and [MiniMax Code](./minimax-code).
...
Use `phaseo run <cline|kilo|omp|muse|minimax>` for the explicit form. Phaseo flags come before `--`; harness flags come after it. Use `--catalog all` to sync all active Phaseo text/chat models, or preview without creating a credential with:

```bash
phaseo cline --model openai/gpt-5.6-terra --dry-run --json
```

Missing Cline, Kilo Code, and oh-my-pi installations are offered interactively. Muse Code and MiniMax Code use their official installers when the command is missing. On Windows, MiniMax Code uses its official PowerShell installer; use `--skip-install` when you manage either harness separately. The launchers do not overwrite existing Cline, Kilo, oh-my-pi, Muse Code, or MiniMax Code configuration.
