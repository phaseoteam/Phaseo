...
`https://docs.phaseo.ai/mcp`

## What it gives you

- live search across the Phaseo docs
- full page content when an agent needs more detail
- access to the hosted [skill.md](https://docs.phaseo.ai/skill.md) capability summary
...
```bash
codex mcp add phaseoDocs --url https://docs.phaseo.ai/mcp
```
...
```bash
claude mcp add --transport http phaseo-docs https://docs.phaseo.ai/mcp
```

Make it available across all projects:

```bash
claude mcp add --transport http --scope user phaseo-docs https://docs.phaseo.ai/mcp
```
