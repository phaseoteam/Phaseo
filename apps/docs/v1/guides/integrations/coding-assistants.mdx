...
| You want to… | Use |
| --- | --- |
| Launch Cline, Kilo Code, oh-my-pi, Muse Code, or MiniMax Code with isolated temporary settings | `phaseo cline`, `phaseo kilo`, `phaseo omp`, `phaseo muse`, or `phaseo minimax` |
...
Supported integration names are `codex`, `claude-code`, `opencode`, `deepseek-harness`, `pi`, `openclaw`, `hermes`, `aider`, `cline`, `roo-code`, `kilo-code`, `continue`, `cursor`, and `zed`. The isolated CLI runners are `cline`, `kilo`, `omp`, `muse`, and `minimax`.
...
Use the dedicated [MiniMax Code guide](./minimax-code.mdx) for the `mcode` CLI. `phaseo minimax` creates a temporary MiniMax provider with the Phaseo OpenAI-compatible endpoint, launches the CLI, and removes the temporary data directory when the session ends. MiniMax Code also supports ACP through `mcode acp` when an editor or host speaks ACP directly.
