...
Use `ai-stats:image_generation` when the model should be able to create an image while it is answering. Phaseo exposes a managed tool to the model, executes the image request server-side, and feeds the image result back into the model loop.
