...
| Subagent | `ai-stats:subagent` | Delegating focused work to a worker model |
...
If the model calls `ai-stats:web_search`, Phaseo executes that call automatically. If it calls `save_research_item`, your application receives the function call and must return the function result in a follow-up request.
