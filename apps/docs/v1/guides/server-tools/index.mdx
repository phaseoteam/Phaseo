...
| --- | --- | --- |
| Datetime | `phaseo:datetime` | Current date and time in up to 5 timezones |
...
| Fusion | `phaseo:fusion` | Parallel analysis followed by model synthesis |
| Search Models | `phaseo:search_models` | Finding suitable models from the live catalogue |
| Image Generation | `phaseo:image_generation` | Creating images from the model tool loop |
| Apply Patch | `phaseo:apply_patch` | Producing validated file patches on the Responses API |
...
curl https://api.phaseo.app/v1/chat/completions \
  ...
  -d '{
    ...
    "tools": [
      ...
      { "type": "phaseo:datetime", "parameters": { "timezones": ["Europe/London", "UTC"] } }
    ]
  }'
```
...
<Columns cols={2}>
  ...
  <Card title="Fusion" icon="combine" href="./fusion">
	Run parallel model analysis and synthesize the findings.
  </Card>
	<Card title="Search Models" icon="list-filter" href="./search-models">
  ...
  <Card title="Image Generation" icon="image" href="./image-generation">
	...
	<Card title="Apply Patch" icon="file-diff" href="./apply-patch">
