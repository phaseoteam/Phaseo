...
Use `ai-stats:advisor` when the main model should be able to ask another model for a review, plan, sanity check, or specialist opinion before it finishes.
...
At most one Advisor entry may omit `name`. If you force `tool_choice: "ai-stats:advisor"` with multiple advisors, Phaseo maps that alias to the first configured Advisor.
