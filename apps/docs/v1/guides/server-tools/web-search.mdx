...
<Note>
TinyFish Search is available as an optional managed engine. Provider-native search remains available through `engine: "native"` on supported routes.
</Note>
...
| --- | --- | --- | --- |
| `engine` | string | `exa` | Search engine: `auto`, `native`, `exa`, `parallel`, `firecrawl`, `perplexity`, or `tinyfish`. |
...
| `max_uses` | integer | `10` | Maximum search calls across the server-tool loop. |
...
| `language` | string | none | TinyFish language hint, such as `en`. |
| `page` | integer | `0` | TinyFish result page, from `0` through `10`. |
...
| `tinyfish` | Uses TinyFish Search when `TINYFISH_API_KEY` is configured. Supports localized, paginated ranked results and translates `allowed_domains` / `excluded_domains` into search operators. TinyFish Search does not supply full page text; use `phaseo:web_fetch` when fuller content is needed. |
...
Enable the engine by configuring `TINYFISH_API_KEY` in your Phaseo gateway runtime secrets, then select it in the tool parameters:
...
`language` provides a locale hint and `page` selects a result page from `0` through `10`. TinyFish supports both domain-filter lists; Phaseo converts them into search operators. Results include URLs, titles, and snippets/highlights. Use `phaseo:web_fetch` when the model needs fuller text from a result.
...
Perplexity and Firecrawl accept either `allowed_domains` or `excluded_domains` in a single search call, not both. Perplexity accepts up to 20 domain filters per request and maps excluded domains to its denylist format. TinyFish translates both arrays to `site:` and `-site:` operators in the query.

TinyFish Search is free on its published plans, so Phaseo does not add a provider usage charge for `engine: "tinyfish"`; normal Phaseo request and token pricing still applies.
