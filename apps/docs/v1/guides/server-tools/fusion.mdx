...
Use Fusion when one answer benefits from several model perspectives. In the Playground, Fusion configures multiple named `ai-stats:advisor` tools for analysis and an optional judge model for synthesis guidance.

Fusion is a convenience workflow built on Advisor. There is no separate `ai-stats:fusion` API tool type; API clients can create the same behavior by adding multiple named Advisor tools.
