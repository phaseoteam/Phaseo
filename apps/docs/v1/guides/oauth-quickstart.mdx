...
1. Go to [Settings → OAuth Apps](https://gateway.aistats.ai/settings/oauth-apps)
...
```typescript
const authUrl = new URL('https://gateway.aistats.ai/oauth/consent');
...
```typescript
const response = await fetch(
  'https://gateway.aistats.ai/v1/chat/completions',
  {
    method: 'POST',
    headers: {
...
- [API Reference](/v1/api-reference/endpoint/oauth) - Detailed OAuth endpoint documentation
- [Token Refresh Guide](/v1/developers/oauth-token-refresh) - Handling token expiration
- [Security Best Practices](/v1/developers/best-practices) - Production security checklist
