...
Endpoint subscriptions can target Batch and Video events independently. Use namespaced event types such as `batch.completed` or `video.failed`; generic `job.*` event types remain supported and subscribe to the matching phase for both job types.
...
After saving an endpoint, use **Send test event** in Settings to deliver a signed `webhook.test` payload. Test deliveries make one attempt and are not retried or added to job delivery history.
