...
- `engine: "auto"` resolves to managed Exa search. `engine: "exa"`, `engine: "parallel"`, `engine: "firecrawl"`, and `engine: "tinyfish"` run managed gateway search when the matching provider key is configured.
- TinyFish Search supports localized, paginated ranked results and is free on its published plans; use `language` and `page` in the tool parameters when needed.
...
- Advisor is gateway-managed and works across supported text models. The calling model receives an `phaseo_advisor` tool, or a named variant such as `phaseo_advisor_reviewer`, and the gateway executes the Advisor request.
