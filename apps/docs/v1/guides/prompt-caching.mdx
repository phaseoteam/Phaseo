...
AI Stats accepts an OpenRouter-style top-level `cache_control` hint on Chat Completions, Responses, and Anthropic Messages requests:
...
Use `ttl: "5m"` for short-lived shared context and `ttl: "1h"` when the provider and model support longer-lived prompt cache entries. A top-level cache control is treated as an automatic/default cache policy by providers that support it.
...
| Field | Meaning |
| --- | --- |
| `input_tokens_details.cached_tokens` | Cached input tokens read from a provider prompt cache. |
| `output_tokens_details.cached_tokens` | Cached input tokens written to a provider prompt cache. |
| `cached_read_text_tokens` | Pricing meter for cache reads. This is cached input text reused from a provider cache. |
| `cached_write_text_tokens` | Pricing meter for cache writes when the provider has one cache-write price. |
