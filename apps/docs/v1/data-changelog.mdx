---
title: "Data Changelog"
description: "Updates to our model and organisation database."
rss: true
---
<Update label="1st June 2026">
# CrofAI Source Sync and Announced Model Tracking

We reconciled CrofAI's live pricing page and `/v1/models` feed against the catalog, and added a newly announced NVIDIA base model in pre-release tracking.

This update includes:

- CrofAI pricing and quantization refreshes for currently listed DeepSeek and GLM routes, based on CrofAI's live pricing page payload on 1st June 2026.
- Retired tracking for [Greg](https://ai-stats.phaseo.app/models/crofai/greg) and end-dated CrofAI-specific provider rows that no longer appear in CrofAI's live model feed.
- Added [Nemotron 3 Ultra](https://ai-stats.phaseo.app/models/nvidia/nemotron-3-ultra-550b-a55b) in `Announced` state using NVIDIA's public pre-release documentation for the base checkpoint.

</Update>

<Update label="1st June 2026">
# MiniMax M3 Release

We promoted MiniMax M3 from placeholder tracking to live catalog coverage after MiniMax's official release on 1st June 2026.

This update includes:

- [MiniMax M3](https://ai-stats.phaseo.app/models/minimax/minimax-m3) moved from `Rumoured` to `Available`.
- Official MiniMax API routing and pricing coverage for `MiniMax-M3`, including the launch pricing split for requests at or below 512K input tokens and the separate long-context tier above 512K tokens.
- Updated modality and context metadata so the model is tracked as a long-context multimodal text model with text, image, and video input support.
- Added provider-specific MiniMax M3 routing and pricing coverage for NovitaAI and Venice, including Novita's published context-based pricing split and Venice's reduced-context 500K route.
- Added Nebius Token Factory pricing and routing coverage for `nvidia/Cosmos3-Super-Reasoner` from the live endpoint information card.

</Update>

<Update label="16th May 2026">
# Provider Activation and Regional Coverage Refresh

We refreshed provider routing metadata, added new regional provider variants, and expanded text-to-speech and pricing coverage across the catalog.

This update includes:

- New regional provider entries for [OpenAI EU](https://ai-stats.phaseo.app/providers/openai-eu) and [Google Vertex EU](https://ai-stats.phaseo.app/providers/google-vertex-eu), plus a fast Nebius Token Factory variant for gateway routing.
- New [Grok TTS](https://ai-stats.phaseo.app/models/x-ai/grok-tts) catalog coverage with `audio.speech` pricing tracked on a per-character meter.
- New provider pricing coverage for OVHcloud and Scaleway model routes, including GPT OSS, Qwen, Mistral, and Llama offerings where current public pricing is available.
- Broader provider status refreshes across Venice, Weights & Biases, Together, SiliconFlow, Bedrock, Alibaba Cloud, and other catalog entries so active routing availability is better aligned with current provider behavior.

</Update>

<Update label="15th May 2026">
# Rumoured Model Tracking

These are not confirmed public releases. They are currently tracked as `Rumoured` based on previously or currently available public information, and no confirmed release date is recorded.

- [Aion 3.0](https://ai-stats.phaseo.app/models/aion-labs/aion-3.0)
- [Step 3.6](https://ai-stats.phaseo.app/models/stepfun/step-3.6)

</Update>

<Update label="7th May 2026">
# Model Additions and Catalog Refreshes

We added several new models and refreshed provider mappings to keep discovery, pricing, and lifecycle data aligned with current releases.

### Google

- [Gemini 3.1 Flash-Lite](https://ai-stats.phaseo.app/models/google/gemini-3.1-flash-lite)

![Gemini 3.1 Flash-Lite](../images/gemini-3.1-flash-lite.png)

### OpenAI

- [GPT Realtime 2](https://ai-stats.phaseo.app/models/openai/gpt-realtime-2)
- [GPT Realtime Translate](https://ai-stats.phaseo.app/models/openai/gpt-realtime-translate)
- [GPT Realtime Whisper](https://ai-stats.phaseo.app/models/openai/gpt-realtime-whisper)

### Moonshot AI

- [Kimi K2](https://ai-stats.phaseo.app/models/moonshot-ai/kimi-k2) catalog metadata refreshed alongside the new additions.

</Update>

<Update label="6th May 2026">
# Model Additions

### xAI

- [Grok Imagine Image Quality](https://ai-stats.phaseo.app/models/x-ai/grok-imagine-image-quality)

</Update>

<Update label="5th May 2026">
# Model Additions

### OpenAI

- [chat-latest](https://ai-stats.phaseo.app/models/openai/chat-latest)

</Update>

<Update label="30th April 2026">
# Withheld Model Update and Benchmark Refresh

We refreshed benchmark metadata, model lifecycle data, and pricing checks across the catalog, and recorded a newly announced withheld OpenAI model.

### OpenAI

- [GPT 5.5 Cyber](https://ai-stats.phaseo.app/models/openai/gpt-5.5-cyber) was announced on 30th April 2026 and is currently marked `Withheld`, so no public release date is recorded.

This update includes:

- Missing benchmark metadata added for recent evaluation sets including `investment-banking-modeling` and `expert-swe`.
- Benchmark ranks recomputed after the latest model additions so standings stay consistent.
- Model statuses refreshed and CrofAI pricing/discovery alerts tightened.

</Update>

<Update label="29th April 2026">
# Model Additions

### Mistral

- [Mistral Medium 3.5](https://ai-stats.phaseo.app/models/mistral/mistral-medium-3.5)

![Mistral Medium 3.5](../images/mistral-medium-3.5.png)

### IBM

- [IBM Granite 4.1 3B](https://ai-stats.phaseo.app/models/ibm/granite-4.1-3b)
- [IBM Granite 4.1 8B](https://ai-stats.phaseo.app/models/ibm/granite-4.1-8b)
- [IBM Granite 4.1 30B](https://ai-stats.phaseo.app/models/ibm/granite-4.1-30b)

![IBM Granite 4.1](../images/im-granite-4.1.png)

</Update>

<Update label="21st April 2026">
# Model Additions

### OpenAI

- [GPT Image 2](https://ai-stats.phaseo.app/models/openai/gpt-image-2)

![GPT Image 2](../images/gpt-image-2.png)

### Qwen

- [Qwen 3.6 Max Preview](https://ai-stats.phaseo.app/models/qwen/qwen3.6-max-preview)

![Qwen 3.6 Max Preview](../images/qwen-3.6-max-preview.png)

### Moonshot AI

- [Kimi K2.6](https://ai-stats.phaseo.app/models/moonshot-ai/kimi-k2.6)

![Kimi K2.6](../images/kimi-k2.6.png)

</Update>

<Update label="17th April 2026">
# Model Additions

### xAI

- [Grok 4.3](https://ai-stats.phaseo.app/models/x-ai/grok-4.3)

![Grok 4.3](../images/grok-4.3.png)

### Qwen

- [Qwen 3.6 35B A3B](https://ai-stats.phaseo.app/models/qwen/qwen3.6-35b-a3b)

![Qwen 3.6 35B A3B](../images/qwen-3.6-35b-a3b.png)

</Update>

<Update label="16th April 2026">
# Model Additions

### Anthropic

- [Claude Opus 4.7](https://ai-stats.phaseo.app/models/anthropic/claude-opus-4-7)

![Claude Opus 4.7](../images/claude-opus-4.7.png)

</Update>

<Update label="15th April 2026">
# Model Additions

### Google

- [Gemini 3.1 Flash TTS Preview](https://ai-stats.phaseo.app/models/google/gemini-3-1-flash-tts-preview)

![Gemini 3.1 Flash TTS Preview](../images/gemini-3.1-flash-tts-preview.png)

</Update>

<Update label="14th April 2026">
# Model Additions

### Google

- [Gemini Robotics ER 1.6 Preview](https://ai-stats.phaseo.app/models/google/gemini-robotics-er-1.6-preview)

![Gemini Robotics ER 1.6 Preview](../images/gemini-robotics-1.6-er-preview.png)

### OpenAI

- [GPT 5.4 Cyber](https://ai-stats.phaseo.app/models/openai/gpt-5.4-cyber)

![GPT 5.4 Cyber](../images/gpt-5.4-cyber.png)

</Update>

<Update label="8th April 2026">
# Model Additions

### Meta

- [Muse Spark](https://ai-stats.phaseo.app/models/meta/muse-spark)
- [Muse Spark Contemplating](https://ai-stats.phaseo.app/models/meta/muse-spark-contemplating)

![Meta Muse Spark](../images/meta-muse-spark.png)

</Update>

<Update label="7th April 2026">
# Model Additions

### Anthropic

- [Claude Mythos Preview](https://ai-stats.phaseo.app/models/anthropic/claude-mythos-preview)

![Claude Mythos Preview](../images/claude-mythos-preview.png)

### Z AI

- [GLM 5.1](https://ai-stats.phaseo.app/models/z-ai/glm-5.1)

![GLM 5.1](../images/glm-5.1.png)

</Update>

<Update label="2nd April 2026">
# Model Additions

### Google

- [Gemma 4 26B A4B](https://ai-stats.phaseo.app/models/google/gemma-4-26b-a4b)
- [Gemma 4 31B](https://ai-stats.phaseo.app/models/google/gemma-4-31b)

![Gemma 4](../images/google-gemma-4.png)

</Update>

<Update label="1st April 2026">
# Model Additions

### Z AI

- [GLM 5V Turbo](https://ai-stats.phaseo.app/models/z-ai/glm-5v-turbo)

![GLM 5V Turbo](../images/glm-5v-turbo.png)

### Qwen

- [Qwen 3.6 Plus](https://ai-stats.phaseo.app/models/qwen/qwen3.6-plus)

![Qwen 3.6 Plus](../images/qwen-3.6-plus.png)

</Update>

<Update label="31st March 2026">
# Model Additions

### Google

- [Veo 3.1 Lite Preview](https://ai-stats.phaseo.app/models/google/veo-3.1-lite-preview)

![Veo 3.1 Lite Preview](../images/veo-3.1-lite.png)

</Update>

<Update label="19th March 2026">
# Model Additions

### Xiaomi

- [MiMo V2 Pro](https://ai-stats.phaseo.app/models/xiaomi/mimo-v2-pro-2026-03-18)
- [MiMo V2 Omni](https://ai-stats.phaseo.app/models/xiaomi/mimo-v2-omni-2026-03-18)

![MiMo V2 Pro + Omni](../images/mimo-v2-omni-pro.png)

- [MiMo V2 TTS](https://ai-stats.phaseo.app/models/xiaomi/mimo-v2-tts-2026-03-18)

![MiMo V2 TTS](../images/mimo-v2-tts.png)

### Cursor

- [Cursor Composer 2](https://ai-stats.phaseo.app/models/cursor/composer-2-2026-03-19)
![Cursor Composer 2](../images/composer-2.png)

</Update>

<Update label="18th March 2026">
# Model Additions

### MiniMax

- [MiniMax M2.7](https://ai-stats.phaseo.app/models/minimax/minimax-m2-7-2026-03-18)

![MiniMax M2.7](../images/minimax-m2.7.png)

</Update>

<Update label="17th March 2026">
# Model Additions

### OpenAI

- [GPT 5.4 Mini](https://ai-stats.phaseo.app/models/openai/gpt-5-4-mini)
- [GPT 5.4 Nano](https://ai-stats.phaseo.app/models/openai/gpt-5-4-nano)

![GPT 5.4 Mini + Nano](../images/gpt-5.4-mini-nano.png)

### Mistral

- [Mistral Moderation 2](https://ai-stats.phaseo.app/models/mistral/mistral-moderation-2-2026-03-17)

![Mistral Moderation 2](../images/mistral-moderation-2.png)

</Update>

<Update label="16th March 2026">
# Model Additions

### Mistral

- [Leanstral](https://ai-stats.phaseo.app/models/mistral/leanstral-2603-2026-03-17)
- [Mistral Small 4](https://ai-stats.phaseo.app/models/mistral/mistral-small-4-2026-03-17)

![Leanstral](../images/leanstral.png)
![Mistral Small 4](../images/mistral-small-4.png)

</Update>

<Update label="15th March 2026">
# Model Additions

### Z AI

- [GLM 5 Turbo](https://ai-stats.phaseo.app/models/z-ai/glm-5-turbo-2026-02-11)

![GLM 5 Turbo](../images/glm-5-turbo.png)

</Update>

<Update label="12th March 2026">
# Model Additions

### NVIDIA

- [Nemotron 3 Super](https://ai-stats.phaseo.app/models/nvidia/nemotron-3-super-2026-03-11)

![Nemotron 3 Super](../images/nemotron-3-super.png)

</Update>

<Update label="11th March 2026">
# Model Additions

### xAI

- [Grok 4.20](https://ai-stats.phaseo.app/models/x-ai/grok-4-20-2026-02-17)

![Grok 4.20](../images/grok-4.2.png)

</Update>

<Update label="10th March 2026">
# Model Additions

### Google

- [Gemini Embedding 2 Preview](https://ai-stats.phaseo.app/models/google/gemini-embedding-2-preview-2026-03-10)

![Gemini Embedding 2 Preview](../images/gemini-embedding-2-preview.png)

</Update>

<Update label="5th March 2026">
# Model Additions

### OpenAI

- [GPT 5.4](https://ai-stats.phaseo.app/models/openai/gpt-5-4)
- [GPT 5.4 Pro](https://ai-stats.phaseo.app/models/openai/gpt-5-4-pro)

![GPT 5.4](../images/gpt-5-4.png)

</Update>

<Update label="3rd March 2026">
# Model Additions

### Google

- [Gemini 3.1 Flash Lite Preview](https://ai-stats.phaseo.app/models/google/gemini-3-1-flash-lite-preview-2026-03-03)

![Gemini 3.1 Flash Lite Preview](../images/gemini-3.1-flash-lite.png)

</Update>

<Update label="26th February 2026">
# Model Additions

### Google

- [Gemini 3.1 Flash Image Preview (Nano Banana 2)](https://ai-stats.phaseo.app/models/google/gemini-3-1-flash-image-preview-2026-02-26)

![Nano Banana 2](../images/nano-banana-2.png)

</Update>

<Update label="24th February 2026">
# Model Additions

### Inception

- [Mercury Edit 2](https://ai-stats.phaseo.app/models/inception/mercury-edit-2)

</Update>

<Update label="23rd February 2026">
# Model Additions

### OpenAI

- [GPT Realtime 1.5](https://ai-stats.phaseo.app/models/openai/gpt-realtime-1-5-2026-02-23)
- [GPT Audio 1.5](https://ai-stats.phaseo.app/models/openai/gpt-audio-1-5-2026-02-23)

![OpenAI 23rd Feb](../images/OpenAI-23rd-feb.png)

</Update>

<Update label="19th February 2026">
# Model Additions

### Google

- [Gemini 3.1 Pro Preview](https://ai-stats.phaseo.app/models/google/gemini-3-1-pro-preview-2026-02-19)

![Gemini 3.1 Pro Preview](../images/gemini-3.1-pro.png)

</Update>

<Update label="18th February 2026">
# Model Additions

### Anthropic

- [Claude Sonnet 4.6](https://ai-stats.phaseo.app/models/anthropic/claude-sonnet-4-6-2026-02-17)

![Claude Sonnet 4.6](../images/sonnet-4.6.png)

</Update>

<Update label="30th January 2026">
# Model Additions

### Z AI

- [GLM 4.5 Air X](https://ai-stats.phaseo.app/models/z-ai/glm-4-5-air-x)
- [GLM 4.5 X](https://ai-stats.phaseo.app/models/z-ai/glm-4-5-x)

### xAI

- [Grok Imagine Image](https://ai-stats.phaseo.app/models/x-ai/grok-imagine-image-2026-01-29)
- [Grok Imagine Video](https://ai-stats.phaseo.app/models/x-ai/grok-imagine-video-2026-01-29)

### Qwen

- [Qwen 3 Max Thinking](https://ai-stats.phaseo.app/models/qwen/qwen-3-max-thinking-2026-01-26)

### Moonshot AI

- [Kimi K2.5](https://ai-stats.phaseo.app/models/moonshot-ai/kimi-k2-5-2026-01-27)

### MiniMax

- [MiniMax M2 Her](https://ai-stats.phaseo.app/models/minimax/minimax-m2-her-2026-01-24)

</Update>

<Update label="19th January 2026">
# Model Additions

- Multiple model additions: [Database Monitor](https://ai-stats.phaseo.app/monitor)

</Update>

<Update label="3rd January 2026">
# Organisation Additions

- [Relace](https://ai-stats.phaseo.app/organisations/relace)
- [Arcee AI](https://ai-stats.phaseo.app/organisations/arcee-ai)
- [Liquid AI](https://ai-stats.phaseo.app/organisations/liquid-ai)
- [Sourceful](https://ai-stats.phaseo.app/organisations/sourceful)
- [Prime Intellect](https://ai-stats.phaseo.app/organisations/prime-intellect)
- [Naver HyperClova](https://ai-stats.phaseo.app/organisations/naver-hyperclova)

# Model Additions

### MiniMax

- [Music 2.0](https://ai-stats.phaseo.app/models/minimax/music-2-0-2025-10-29)
- [Speech 2.6](https://ai-stats.phaseo.app/models/minimax/speech-2-6-2025-10-29)

### Z AI

- [GLM 4.7](https://ai-stats.phaseo.app/models/z-ai/glm-4-7-2025-12-22)

### ByteDance

- [Seed 1.6](https://ai-stats.phaseo.app/models/bytedance/seed-1-6)
- [Seed 1.6 Flash](https://ai-stats.phaseo.app/models/bytedance/seed-1-6-flash)
- [Seed 1.8](https://ai-stats.phaseo.app/models/bytedance/seed-1-8-2025-12-18)
- [Seedream 4.5](https://ai-stats.phaseo.app/models/bytedance/seedream-4-5)

### Relace

- [Relace Search](https://ai-stats.phaseo.app/models/relace/relace-search)

### Arcee AI

- [Trinity Large](https://ai-stats.phaseo.app/models/arcee-ai/trinity-large)

# API Provider Additions

- [Moonshot AI Turbo](https://ai-stats.phaseo.app/api-providers/moonshotaiturbo)
- [Nebius Token Factory](https://ai-stats.phaseo.app/api-providers/nebiustokenfactory)
- [Crusoe](https://ai-stats.phaseo.app/api-providers/crusoe)
- [Liquid](https://ai-stats.phaseo.app/api-providers/liquid)
- [Mancer](https://ai-stats.phaseo.app/api-providers/mancer)
- [Relace](https://ai-stats.phaseo.app/api-providers/relace)
- [Xiaomi](https://ai-stats.phaseo.app/api-providers/xiaomi)
- [Morph](https://ai-stats.phaseo.app/api-providers/morph)
- [Phala](https://ai-stats.phaseo.app/api-providers/phala)
- [Z AI](https://ai-stats.phaseo.app/api-providers/zai)
- [Aion Labs](https://ai-stats.phaseo.app/api-providers/aionlabs)
- [Clarifai](https://ai-stats.phaseo.app/api-providers/clarifai)
- [Friendli](https://ai-stats.phaseo.app/api-providers/friendli)
- [GMI Cloud](https://ai-stats.phaseo.app/api-providers/gmicloud)
- [Fireworks](https://ai-stats.phaseo.app/api-providers/fireworks)
- [Inception](https://ai-stats.phaseo.app/api-providers/inception)
- [Sambanova](https://ai-stats.phaseo.app/api-providers/sambanova)
- [Sourceful](https://ai-stats.phaseo.app/api-providers/sourceful)
- [Atlas Cloud](https://ai-stats.phaseo.app/api-providers/atlascloud)
- [Cloudflare](https://ai-stats.phaseo.app/api-providers/cloudflare)
- [Hyperbolic](https://ai-stats.phaseo.app/api-providers/hyperbolic)
- [Infermatic](https://ai-stats.phaseo.app/api-providers/infermatic)
- [Inflection](https://ai-stats.phaseo.app/api-providers/inflection)
- [Perplexity](https://ai-stats.phaseo.app/api-providers/perplexity)
- [Featherless](https://ai-stats.phaseo.app/api-providers/featherless)
- [SiliconFlow](https://ai-stats.phaseo.app/api-providers/siliconflow)
- [ByteDance Seed](https://ai-stats.phaseo.app/api-providers/bytedanceseed)
- [Black Forest Labs](https://ai-stats.phaseo.app/api-providers/blackforestlabs)
- [MiniMax Lightning](https://ai-stats.phaseo.app/api-providers/minimaxlightning)
- [Weights & Biases](https://ai-stats.phaseo.app/api-providers/weightsandbiases)

</Update>

<Update label="31st December 2025">
# Model Additions

### LG

- [K EXAONE](https://ai-stats.phaseo.app/models/lg/k-exaone-2025-12-31)

</Update>

<Update label="29th December 2025">
# Model Additions

### Naver-HyperClova

- [HyperClova X Seed Omni 8B](https://ai-stats.phaseo.app/models/naver-hyperclova/hyperclova-x-seed-omni-8b-2025-12-29)
- [HyperClova X Seed Think 32B](https://ai-stats.phaseo.app/models/naver-hyperclova/hyperclova-x-seed-think-32b-2025-12-29)

</Update>

<Update label="23rd December 2025">
# Model Additions

### Minimax

- [MiniMax M2.1](https://ai-stats.phaseo.app/models/minimax/minimax-m2-1-2025-12-23)

</Update>

<Update label="22nd December 2025">
# Model Additions

### Z-AI

- [GLM 4.7](https://ai-stats.phaseo.app/models/z-ai/glm-4-7-2025-12-22)

</Update>

<Update label="19th December 2025">
# Model Additions

### OpenAI

- [GPT 5.2 Codex](https://ai-stats.phaseo.app/models/openai/gpt-5-2-codex-2025-12-18)

### Mistral

- [OCR 3](https://ai-stats.phaseo.app/models/mistral/ocr-3-2025-12-18)

### Z-AI

- [GLM 4.7](https://ai-stats.phaseo.app/models/z-ai/glm-4-7-2025-12-19)

</Update>

<Update label="18th December 2025">
# Model Additions

### Google

- [Gemini 3 Flash Preview](https://ai-stats.phaseo.app/models/google/gemini-3-flash-preview-2025-12-17)

</Update>

<Update label="17th December 2025">
# Model & Organisation Additions

## Organisation Additions

- [Xiaomi](https://ai-stats.phaseo.app/organisations/xiaomi) - A leading technology company expanding into AI
- [AllenAI](https://ai-stats.phaseo.app/organisations/allenai) - A research institute focused on artificial intelligence

## Model Additions

### OpenAI

- [GPT Image 1.5](https://ai-stats.phaseo.app/models/openai/gpt-image-1-5-2025-12-16)
- [ChatGPT Image Latest](https://ai-stats.phaseo.app/models/openai/chatgpt-image-latest-2025-12-16)

### Xiaomi

- [MiMo V2 Flash](https://ai-stats.phaseo.app/models/xiaomi/mimo-v2-flash-2025-12-16)

### AllenAI

- [Bolmo 1B](https://ai-stats.phaseo.app/models/allenai/bolmo-1b-2025-12-15)
- [Bolmo 7B](https://ai-stats.phaseo.app/models/allenai/bolmo-7b-2025-12-15)
- [Molmo 2 4B](https://ai-stats.phaseo.app/models/allenai/molmo-2-4b-2025-12-16)
- [Molmo 2 8B](https://ai-stats.phaseo.app/models/allenai/molmo-2-8b-2025-12-16)
- [Olmo 3.1 32B Instruct](https://ai-stats.phaseo.app/models/allenai/olmo-3-1-32b-instruct-2025-12-12)
- [Olmo 3.1 32B Think](https://ai-stats.phaseo.app/models/allenai/olmo-3-1-32b-think-2025-12-12)
- [Olmo 3 32B Think](https://ai-stats.phaseo.app/models/allenai/olmo-3-32b-think-2025-11-20)
- [Olmo 3 7B Instruct](https://ai-stats.phaseo.app/models/allenai/olmo-3-7b-instruct-2025-11-20)
- [Olmo 3 7B Think](https://ai-stats.phaseo.app/models/allenai/olmo-3-7b-think-2025-11-20)

</Update>

<Update label="15th December 2025">
# Model Additions

### OpenAI

- [GPT 5.2 Mini](https://ai-stats.phaseo.app/models/openai/gpt-5-2-mini-2025-12-15)
- [GPT 4o Mini TTS](https://ai-stats.phaseo.app/models/openai/gpt-4o-mini-tts-2025-12-15)
- [GPT 4o Mini Transcribe](https://ai-stats.phaseo.app/models/openai/gpt-4o-mini-transcribe-2025-12-15)
- [GPT Realtime Mini](https://ai-stats.phaseo.app/models/openai/gpt-realtime-mini-2025-12-15)
- [GPT Audio Mini](https://ai-stats.phaseo.app/models/openai/gpt-audio-mini-2025-12-15)

### Anthropic

- [Claude 4.7 Sonnet](https://ai-stats.phaseo.app/models/anthropic/claude-sonnet-4-7-2025-12-15)

</Update>

<Update label="12th December 2025">
# Model Additions

### OpenAI

- [GPT 5.2](https://ai-stats.phaseo.app/models/openai/gpt-5-2-2025-12-11)
- [GPT 5.2 Chat](https://ai-stats.phaseo.app/models/openai/gpt-5-2-chat-2025-12-11)
- [GPT 5.2 Pro](https://ai-stats.phaseo.app/models/openai/gpt-5-2-pro-2025-12-11)
- [GPT 5.2 Mini](https://ai-stats.phaseo.app/models/openai/gpt-5-2-mini-2025-12-11)

### Cohere

- [Rerank V4 Pro](https://ai-stats.phaseo.app/models/cohere/rerank-v4-0-pro-2025-12-11)
- [Rerank V4 Fast](https://ai-stats.phaseo.app/models/cohere/rerank-v4-0-fast-2025-12-11)

</Update>

<Update label="10th December 2025">
# Model Additions

### Qwen

- [Qwen 3 Omni Flash](https://ai-stats.phaseo.app/models/qwen/qwen-3-omni-flash-2025-12-08)

</Update>

<Update label="9th December 2025">
# Model Additions

### Mistral

- [Devstral 2](https://ai-stats.phaseo.app/models/mistral/devstral-2-2025-12-09)
- [Devstral Small 2](https://ai-stats.phaseo.app/models/mistral/devstral-small-2-2025-12-09)

### Nomos

- [Nomos 1](https://ai-stats.phaseo.app/models/nomos/nomos-1-2025-12-09)

</Update>

<Update label="8th December 2025">
# Model Additions

### Essential AI

- [Essential AI Rnj-1](https://ai-stats.phaseo.app/models/essential-ai/rnj-1-2025-12-06)

### Nous

- [Nous Research Hermes 4.3](https://ai-stats.phaseo.app/models/nous/hermes-4-3-36b-2025-12-03)

### AI21

- [Jamba Reasoning 3B](https://ai-stats.phaseo.app/models/ai21/jamba-reasoning-3b-2025-10-08)
- [GLM 4.6V](https://ai-stats.phaseo.app/models/ai21/glm-4-6v-2025-12-08)
- [GLM 4.6V Flash](https://ai-stats.phaseo.app/models/ai21/glm-4-6v-flash-2025-12-08)
- [Qwen 3 TTS](https://ai-stats.phaseo.app/models/qwen/qwen-3-tts-2025-12-05)
- [Gemini 3 Flash Image](https://ai-stats.phaseo.app/models/google/gemini-3-flash-image-2025-12-08)

</Update>

<Update label="2nd December 2025">
# Model Additions

### Mistral

- [Mistral Large 3 675B](https://ai-stats.phaseo.app/models/mistral/mistral-large-3-675b-2025-12-02)
- [Ministral 3 14B](https://ai-stats.phaseo.app/models/mistral/ministral-3-14b-2025-12-02)
- [Ministral 3 8B](https://ai-stats.phaseo.app/models/mistral/ministral-3-8b-2025-12-02)
- [Ministral 3 3B](https://ai-stats.phaseo.app/models/mistral/ministral-3-3b-2025-12-02)

### Amazon

- [Nova 2 Lite](https://ai-stats.phaseo.app/models/amazon/nova-2-lite-2025-12-02)
- [Nova 2 Pro](https://ai-stats.phaseo.app/models/amazon/nova-2-pro-2025-12-02)
- [Nova 2 Sonic](https://ai-stats.phaseo.app/models/amazon/nova-2-sonic-2025-12-02)
- [Nova 2 Omni](https://ai-stats.phaseo.app/models/amazon/nova-2-omni-2025-12-02)
- [Nova Multimodal Embeddings](https://ai-stats.phaseo.app/models/amazon/nova-multimodal-embeddings-2025-12-02)

</Update>

<Update label="1st December 2025">
# Model Additions

### DeepSeek

- [Deepseek V3.2 Speciale](https://ai-stats.phaseo.app/models/deepseek/deepseek-v3-2-speciale-2025-12-01)
- [DeepSeek V3.2](https://ai-stats.phaseo.app/models/deepseek/deepseek-v3-2-2025-12-01)

</Update>

<Update label="1st December 2025">
# Introducing The New AI Stats

## Data coverage

At the heart of AI Stats is our database. It is designed to be a highly expansive and flexible store of model data. If you have used AI Stats before, you may know we only used to have text-based models. However, we always wanted to change that, so today we are announcing that we have grown our entire data coverage dramatically to allow us to track more models, such as image, video, and embedding models, in as much detail as possible.

There is still a long road to capture every organisation, model, and benchmark result. We currently sit at ~30% coverage, but we are prioritising expanding this as quickly as possible and aim to reach at least 60% over the next year. We want our database to be the gold standard, whilst still keeping it open source so anyone can contribute. We always welcome contributions to help us get there faster!

### Model Availability

With the introduction of the AI Stats Gateway, we have also reworked how we track model availability. A full subscription plan section now allows you to see all the subscription plans we track, the models they include, and the features they offer, as well as clearly seeing the price. We have also adjusted how we display availability to make it clearer and easier to see where models are available. We plan to continue tweaking this section to make it as useful as possible.

## Data Overhaul

We have revisited what data we store, how we store it, and how we present that data, so we have built a smoother, more reliable experience across every page. It is not perfect yet, and we have plenty of optimisations ahead. We are working hard to adjust certain pages to make them clearer and easier to use. We know there is more to do, and we plan to make trends easier to track, comparisons easier to make, and data more accessible. If you have any feedback on how we can improve – whether that is certain pages and their designs or layouts, or even tracking new data points – please reach out!

</Update>

<Update label="30th June 2025">
# Model Additions

### Tencent

- [Hunyuan A13B Instruct](https://ai-stats.phaseo.app/models/tencent/hunyuan-a13b-instruct)

</Update>

<Update label="30th August 2024">
# Model Additions

### Cohere

- [Command R](https://ai-stats.phaseo.app/models/cohere/command-r)

</Update>
