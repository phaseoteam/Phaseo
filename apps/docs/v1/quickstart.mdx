...
Use `POST /v1/decisions` when your application needs typed answers instead of generated text. Phaseo exposes TypeSafe's Jev 1.13 with the model ID `typesafe/jev-1.13.0`. Send a `state` value and one or more named questions; each answer is returned under the same name.
