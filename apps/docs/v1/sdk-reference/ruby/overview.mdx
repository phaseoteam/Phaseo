...
- `list_models(...)`
- `model_endpoint_capabilities(...)`, `check_model_parameters(...)`
...
`check_model_parameters(...)` uses live provider-route metadata to highlight unsupported or invalid values and return the routes that accept the complete parameter set.
