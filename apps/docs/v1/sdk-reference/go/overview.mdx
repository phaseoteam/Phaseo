...
Install the published [`sdk-go/v2`](https://pkg.go.dev/github.com/phaseoteam/Phaseo/packages/sdk/sdk-go/v3) module.
...
- `GetModels(...)`
- `GetModelEndpointCapabilities(...)`, `CheckModelParameters(...)`
- `ListOrganisations(...)`
- `ListPricingModels(...)`
- `CalculatePricing(...)`
- `ListApiKeys(...)`
...
`CheckModelParameters` uses live provider-route metadata to label each parameter as `supported`, `partial`, `unsupported`, or `unknown`, validate advertised values and ranges, and return the routes that accept the complete parameter set.
