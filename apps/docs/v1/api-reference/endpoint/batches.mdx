...
Persist and poll the gateway-owned response `id`. When AI Stats also returns `native_batch_id`, that value is the provider-native id used for upstream correlation; do not use it in AI Stats polling, websocket, or cancellation URLs unless it is also the gateway `id`.
