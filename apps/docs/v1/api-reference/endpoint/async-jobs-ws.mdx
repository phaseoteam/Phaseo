...
<Note type="warning">
Experimental endpoint. This route is WebSocket-only and currently supports async batch and video lifecycle updates.
</Note>
...
- URL: `wss://api.phaseo.ai/v1/async/{kind}/{id}/ws`
- Supported `kind` values: `batch`, `video`
- Method semantics: WebSocket starts as an HTTP `GET` with `Upgrade: websocket`
- Auth: `Authorization: Bearer YOUR_API_KEY`

Phaseo checks workspace ownership before opening the socket. If the async job is missing or not owned by the caller, the HTTP handshake fails with `404 async_job_not_found_or_not_owned`.
...
The `data` payload follows the same owned async-job shape used by webhook and web UI notifications, including status, routing metadata, file/content links where available, and terminal billing fields for supported kinds.
...
Failed batch and video payloads can also include the same normalized gateway failure fields used by the request error contract when that metadata was captured during execution or reconciliation:

- `upstream_error`
  - normalized upstream provider error object with fields like `code`, `message`, `description`, `param`, and `status`
- `provider_failure_diagnostics`
  - compact classification of the failure category, implicated provider, and optional hint
- `failure_sample`
...
- `provider_candidate_diagnostics`
  - candidate counts and dropped-provider summary data when Phaseo computed a narrowed provider set
...
When `close_on_terminal=true`, Phaseo closes the socket after a terminal `completed`, `failed`, or `cancelled` update. Disable that flag if you want the client to keep the connection open and decide when to close it.
