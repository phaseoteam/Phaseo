...
AI Stats checks workspace ownership before opening the socket. If the async job is missing or not owned by the caller, the HTTP handshake fails with `404 async_job_not_found_or_not_owned`.
...
- `provider_candidate_diagnostics`
  - candidate counts and dropped-provider summary data when AI Stats computed a narrowed provider set
...
When `close_on_terminal=true`, AI Stats closes the socket after a terminal `completed`, `failed`, or `cancelled` update. Disable that flag if you want the client to keep the connection open and decide when to close it.
