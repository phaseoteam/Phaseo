...
Use `POST /v1/decisions` when your application needs structured answers that code can consume directly. Phaseo exposes TypeSafe's Jev 1.13 as `typesafe/jev-1.13.0`.
...
- `model`: the Phaseo model ID. Use `typesafe/jev-1.13.0` for Jev 1.13, or `typesafe/jev-latest` to follow the latest Jev release.
