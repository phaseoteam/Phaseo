...
If you want tuning advice instead of field definitions, use [Inference Parameters](/v1/guides/inference-parameters) and [Sampling and Decoding](../guides/sampling-and-decoding.mdx).
...
| [`prompt_cache_options`](#prompt_cache_options) | `object` | Setting OpenAI prompt-cache mode and TTL controls. |
...
<span id="prompt_cache_options"></span>
<h3 id="parameter-prompt_cache_options"><code>prompt_cache_options</code></h3>

Passes OpenAI prompt-cache controls through supported OpenAI routes.

| Field | Value |
| --- | --- |
| Type | `object` |
| Common fields | `mode`, `ttl` |
| Astra TTL | `30m` |

For GPT-6 Astra, use `{"mode":"explicit","ttl":"30m"}` when you want explicit prompt caching. Phaseo preserves this object through its request normalization and sends it to OpenAI unchanged.
...
| `quantizations` | `string[]` | Require an eligible offer whose catalogue quantization matches one of these values. |

`quantizations` follows the OpenRouter-compatible provider-routing vocabulary and is accepted under either `provider` or `routing`. Matching is case-insensitive and ignores spaces, hyphens, and underscores; unambiguous names such as `float8`/`FP8` and `bfloat16`/`BF16` are aliases. Offers without quantization metadata are excluded whenever this filter is present. If no eligible offer matches, the Gateway returns an error with the requested and currently available quantizations instead of silently routing to a different variant.
...
- [Sampling and Decoding](../guides/sampling-and-decoding.mdx) for how randomness, penalties, and decoding controls change model behavior

## Related pages

- [Inference Parameters](/v1/guides/inference-parameters)
- [Sampling and Decoding](../guides/sampling-and-decoding.mdx)
- [Streaming](../guides/streaming.mdx)
- [Limits](./limits.mdx)
