...
description: "Use `ai-stats:web_fetch` to pull bounded page text into the tool loop for deterministic follow-up extraction and review."
---
...
## 1. Use `ai-stats:web_fetch` for one-page grounding

`ai-stats:web_fetch` is a gateway-managed server tool.
...
1. ask the model to call `ai-stats:web_fetch`
...
`ai-stats:web_fetch` is intentionally bounded.
