...
Persist and poll with the gateway-owned video `id`. `native_video_id`, when present, is provider-native correlation metadata; do not use it for Phaseo polling, websocket, cancellation, or content URLs.
...
Webhook subscriptions accept generic `job.*` events or matching `video.*` events. If you omit `events`, Phaseo subscribes the webhook to terminal `job.completed`, `job.failed`, `job.cancelled`, and `job.expired` notifications. If you provide `events`, at least one event must be valid for video jobs; all-invalid or cross-kind-only lists such as `["batch.completed"]` are rejected instead of being broadened silently.
