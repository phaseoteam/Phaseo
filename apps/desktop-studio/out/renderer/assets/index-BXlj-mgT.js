...
const manifestEntries = Object.entries(logoManifest);
const normalisedKeyMap = /* @__PURE__ */ new Map();
const pathToKeyMap = /* @__PURE__ */ new Map();
...
for (const [key, assets] of manifestEntries) {
  normalisedKeyMap.set(normalise$1(key), key);
  for (const value of Object.values(assets)) {
    if (!value) continue;
    pathToKeyMap.set(value, key);
  }
}
function normalise$1(value) {
  return value.toLowerCase().replace(/[^a-z0-9]/g, "");
}
function lookupKnownId(input) {
  if (!input) return void 0;
  if (logoManifest[input]) {
    return input;
  }
  const normalised = normalise$1(input);
...
function labelFromId(id) {
  ...
  const words = id.replace(/[-_]+/g, " ").split(" ").filter(Boolean).map((word) => word.charAt(0).toUpperCase() + word.slice(1));
  return words.join(" ");
}
function pickVariant(assets, requestedVariant, theme, fallbackToColor) {
  const { color: color2, light, dark } = assets;
  const applyFallbackVariant = (requested, source, fallback) => {
    if (!source) return { src: void 0, variant: requested };
    return {
      src: source,
  ...
  switch (requestedVariant) {
    case "color": {
      if (color2) return { src: color2, variant: "color" };
      if (fallbackToColor) {
        const fallback = theme === "dark" ? dark ?? light : light ?? dark;
        return applyFallbackVariant(theme, fallback ?? color2, color2 ? "color" : theme);
      }
      break;
    }
    case "light": {
      const fallbackVariant = light ? "light" : color2 ? "color" : "dark";
      const src2 = light ?? color2 ?? dark;
      return { src: src2, variant: fallbackVariant };
    }
    case "dark": {
      const fallbackVariant = dark ? "dark" : color2 ? "color" : "light";
      const src2 = dark ?? color2 ?? light;
      return { src: src2, variant: fallbackVariant };
    }
    case "mono": {
      if (theme === "dark") {
        const src2 = dark ?? (fallbackToColor ? color2 ?? light : void 0);
        const variant = dark && src2 === dark ? "dark" : color2 && src2 === color2 ? "color" : "light";
        return { src: src2, variant };
      }
      {
        const src2 = light ?? (fallbackToColor ? color2 ?? dark : void 0);
        const variant = light && src2 === light ? "light" : color2 && src2 === color2 ? "color" : "dark";
        return { src: src2, variant };
      }
    }
    case "auto":
    default: {
      ...
      const src2 = theme === "dark" ? dark ?? (fallbackToColor ? color2 ?? light : void 0) : light ?? (fallbackToColor ? color2 ?? dark : void 0);
      if (src2 === color2) return { src: src2, variant: "color" };
      return { src: src2, variant: fallbackVariant };
    }
  }
  if (color2) return { src: color2, variant: "color" };
...
function isAiStatsGateway(provider) {
  const candidate = [
    provider.name,
    provider.id,
    provider.baseUrl,
...
function inferProviderLogoId(provider) {
...
function resolveDesktopLogo(provider, theme) {
...
function Logo({ provider, size = 18, className = "" }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    light.src ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      "img",
      {
        src: light.src,
        alt,
    ...
    dark.src ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      "img",
      {
        src: dark.src,
        alt,
...
function getGatewayProvider(settings) {
  return settings.providers.find((provider) => provider.id === settings.chatSelection.providerId) ?? settings.providers.find((provider) => provider.kind === "openai-compatible") ?? settings.providers[0] ?? null;
}
const ChatPane = reactExports.memo(function ChatPane2({
  messages,
  isSending,
  settings,
  onSettingsChange,
  ...
  const [syncingProviderId, setSyncingProviderId] = reactExports.useState(null);
  const [syncMessage, setSyncMessage] = reactExports.useState("");
  const [gatewayApiKeyDraft, setGatewayApiKeyDraft] = reactExports.useState("");
  const lastAutoSyncKeyRef = reactExports.useRef("");
  const orderedMessages = reactExports.useMemo(() => messages, [messages]);
  const gatewayProvider = reactExports.useMemo(() => getGatewayProvider(settings), [settings]);
  ...
  const submit = async () => {
    ...
    setDraft("");
    await onSend(content2);
  };
  const applyGatewayPatch = async (patch2) => {
    if (!gatewayProvider) {
      return;
    }
    const nextModels = uniqueNonEmpty(patch2.models ?? gatewayProvider.models);
    ...
    const nextDefaultModelCandidate = (patch2.defaultModel ?? gatewayProvider.defaultModel).trim();
    const nextDefaultModel = nextModels.includes(nextDefaultModelCandidate) ? nextDefaultModelCandidate : nextModels[0] ?? fallbackModel;
    const nextGatewayProvider = {
      ...gatewayProvider,
      ...patch2,
    ...
    const providers = settings.providers.some((provider) => provider.id === nextGatewayProvider.id) ? settings.providers.map((provider) => provider.id === nextGatewayProvider.id ? nextGatewayProvider : provider) : [nextGatewayProvider];
    const nextChatModel = nextGatewayProvider.models.includes(settings.chatSelection.model) ? settings.chatSelection.model : nextGatewayProvider.defaultModel;
    const nextCodeModel = nextGatewayProvider.models.includes(settings.codeSelection.model) ? settings.codeSelection.model : nextGatewayProvider.defaultModel;
  ...
  const saveGatewaySettings = async (options) => {
    if (!gatewayProvider) {
      return;
    }
    const nextApiKey = gatewayApiKeyDraft.trim();
    const alreadySaved = gatewayProvider.baseUrl.trim() === GATEWAY_BASE_URL && gatewayProvider.apiKey.trim() === nextApiKey;
    if (!alreadySaved) {
  ...
  const syncGatewayModels = async (options) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "chat-pane", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "pane-header chat-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-title-line", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Unified Chat" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Gateway-first local chat with web-aligned controls." })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-header-controls", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "active-model-badge", children: [
          gatewayProvider ? /* @__PURE__ */ jsxRuntimeExports.jsx(Logo, { provider: gatewayProvider, size: 18 }) : null,
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: gatewayProvider?.name ?? "AI Stats Gateway" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "header-select-wrap", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "select",
            {
              ...
              children: (gatewayProvider?.models ?? []).map((model) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: model, children: model }, model))
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "secondary-button", onClick: () => setSettingsOpen(true), children: "Settings" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
            ...
            children: syncingProviderId === gatewayProvider?.id ? "Syncing..." : "Sync Models"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "pane-status-badge", children: isSending ? "Thinking..." : "Ready" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "message-list chat-message-list", children: orderedMessages.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "chat-empty-state", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Start a conversation" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Configure your gateway key once, then chat with any synced model." })
    ] }) : orderedMessages.map((message) => /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: `message message-${message.role}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: message.role === "assistant" ? "Assistant" : message.role === "user" ? "You" : "System" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: message.status ?? "done" })
      ] }),
      message.role === "assistant" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "markdown", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Markdown, { children: message.content || "..." }) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: message.content || "..." })
    ] }, message.id)) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-composer-shell", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "composer chat-composer", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "textarea",
        {
          value: draft,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "composer-meta", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "composer-chip", children: [
          gatewayProvider ? /* @__PURE__ */ jsxRuntimeExports.jsx(Logo, { provider: gatewayProvider, size: 14 }) : null,
          gatewayProvider?.name ?? "AI Stats Gateway"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "composer-chip", children: settings.chatSelection.model || "No model" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "primary-button chat-send-button", disabled: !canSend, onClick: () => void submit(), children: "Send" })
    ] }) }),
    settingsOpen ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-overlay", role: "dialog", "aria-modal": "true", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "settings-panel", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "settings-panel-header", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Chat Settings" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-block", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-label", children: "Appearance" }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-block", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-label", children: "Gateway" }),
          gatewayProvider ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "provider-preview-row", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Logo, { provider: gatewayProvider, size: 20 }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: gatewayProvider.name })
          ] }) : null,
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "Gateway URL",
            /* @__PURE__ */ jsxRuntimeExports.jsx(
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "sync-status-text", children: "API key is saved locally as soon as you press Enter or leave the field." }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "provider-actions-row", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                type: "button",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: syncingProviderId === gatewayProvider?.id ? "Syncing..." : "Save + Sync Models"
              }
            )
          ] }),
          syncMessage ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "sync-status-text", children: syncMessage }) : null
        ] })
      ] })
    ] }) : null
  ] });
...
const Sidebar = reactExports.memo(function Sidebar2({
  sessions,
  activeSessionId,
  onCreateSession,
  onSelectSession
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "sidebar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sidebar-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: "/wordmark_light.svg", alt: "AI Stats", className: "wordmark wordmark-light" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: "/wordmark_dark.svg", alt: "AI Stats", className: "wordmark wordmark-dark" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Unified chat" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button", onClick: onCreateSession, type: "button", children: "New Chat" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "sidebar-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-label", children: "Chats" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "session-list", children: sessions.map((session) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          type: "button",
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "session-title", children: session.title }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "session-meta", children: formatSessionTimestamp(session.updatedAt) })
          ]
        }
      ) }, session.id)) })
    ] })
...
function App() {
  ...
  const [messagesBySession, setMessagesBySession] = reactExports.useState({});
  ...
  reactExports.useEffect(() => {
    ...
    const applySystemTheme = () => {
      root2.setAttribute("data-theme", media.matches ? "dark" : "light");
    };
    applySystemTheme();
  ...
  reactExports.useEffect(() => {
    void (async () => {
      try {
        ...
        const chatSessions2 = data.sessions.filter((session) => session.mode === "chat");
        const preferredActiveId = data.activeSessionId && chatSessions2.some((session) => session.id === data.activeSessionId) ? data.activeSessionId : chatSessions2[0]?.id ?? null;
        setSessions(chatSessions2);
    ...
    const removeChunk = window.desktop.onChatChunk((event) => {
      reactExports.startTransition(() => {
        setMessagesBySession((prev) => {
          ...
          return {
            ...prev,
            [event.sessionId]: existing.map(
              (message) => message.id === event.messageId ? { ...message, content: message.content + event.chunk, status: "streaming" } : message
            )
          };
        });
      });
    });
    const removeDone = window.desktop.onChatDone((event) => {
      reactExports.startTransition(() => {
        setMessagesBySession((prev) => {
          const existing = prev[event.sessionId] ?? [];
          return {
            ...prev,
            [event.sessionId]: existing.map(
              (message) => message.id === event.messageId ? { ...message, status: event.status } : message
            )
          };
        });
      });
  ...
  const activeMessages = reactExports.useMemo(() => {
    if (!activeSessionId) {
      return [];
    }
    return messagesBySession[activeSessionId] ?? [];
  }, [activeSessionId, messagesBySession]);
  const chatSessions = reactExports.useMemo(
  ...
  const handleSend = async (content2) => {
  ...
  if (isLoading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "boot-screen", children: "Loading AI Stats Chat..." });
  }
  if (bootError) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "boot-screen", children: [
      "Startup Error: ",
      bootError
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-shell", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "main-pane", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      ChatPane,
      {
