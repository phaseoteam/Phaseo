...
Currently configured project: `ai-stats-public` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/ai-stats-public/SETUP.md` to fill in
   `data/ai-stats-public/INFO.md`.
...
`--project-id ai-stats-public` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/ai-stats-public/`.
