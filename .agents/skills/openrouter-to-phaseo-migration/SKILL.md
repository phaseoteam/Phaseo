---
name: openrouter-to-phaseo-migration
description: End-to-end migration guide for moving an existing OpenRouter integration to Phaseo Gateway. Use when repositories contain openrouter.ai endpoints, sk-or-v1 keys, OPENROUTER_API_KEY env vars, provider-prefixed model IDs, or OpenRouter-specific headers and the user wants a full migration with implementation, validation, and safe rollout.
---

# OpenRouter to Phaseo Migration

Use this skill to complete a full migration from OpenRouter to Phaseo Gateway with minimal risk.

## Outcome
Deliver a working integration that:
- sends traffic to Phaseo (`https://api.phaseo.app/v1`)
- authenticates with `PHASEO_API_KEY`
...
- Build a temporary compatibility map when needed (example: translate `provider/model` aliases to Phaseo IDs).
...
## 4) Handle OpenRouter-only Behaviors
- If code depends on provider routing/preferences, remove or re-implement with Phaseo-supported controls.
...
- all target environments use `PHASEO_API_KEY`
