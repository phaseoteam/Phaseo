# Phaseo AI SDK Provider - Implementation Summary
...
Developer Application (Vercel AI SDK)
          ↓
@phaseo/ai-sdk-provider (Translation Layer)
          ↓
Phaseo Gateway API (/v1/*)
          ↓
...
- Phaseo-specific `providerMetadata` passthrough for request/routing fields
- Default `phaseo` export environment-resolution behavior at module load time
- Phaseo environment resolution for both the factory and default export
...
**Features:**
- Requires explicit `PHASEO_RUN_GATEWAY_TESTS=1` opt-in for paid/live requests
...
- Uses `PHASEO_API_KEY` and `PHASEO_BASE_URL` for live gateway configuration
