...
if (!apiKey) {
	throw new Error("Missing PHASEO_API_KEY in the environment.");
}

const phaseo = createPhaseo({
	apiKey,
	baseURL: process.env.PHASEO_BASE_URL,
});
...
async function run() {
	...
	const embeddingResult = await embed({
		model: phaseo.embeddingModel(embeddingModelId),
