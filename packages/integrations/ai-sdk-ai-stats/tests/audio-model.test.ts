...
describe('AI Stats Vercel AI SDK audio compatibility', () => {
  it('maps experimental_generateSpeech responses from /audio/speech', async () => {
    ...
    const result = await experimental_generateSpeech({
      model: aiStats.speechModel('openai/tts-1', { user: 'user_123' }),
      text: 'Hello from AI Stats',
      voice: 'alloy',
      speed: 1.25,
      outputFormat: 'mp3',
  ...
  it('maps experimental_transcribe responses from /audio/transcriptions and uploads multipart audio', async () => {
    ...
    const result = await experimental_transcribe({
      model: aiStats.transcriptionModel('openai/whisper-1', { user: 'user_123' }),
      audio: audioData,
      providerOptions: {
        openai: {
          language: 'en',
