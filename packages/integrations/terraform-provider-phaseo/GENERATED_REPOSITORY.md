...
Do not commit provider implementation changes directly to the generated repository. Open changes against the monorepo; an hourly workflow tests the exported provider and synchronizes its `main` branch automatically, and maintainers can trigger an immediate manual sync. The sync workflow uses the `PROVIDER_SYNC_TOKEN` repository secret because GitHub requires the separate Workflows permission when an export updates `.github/workflows` files.
