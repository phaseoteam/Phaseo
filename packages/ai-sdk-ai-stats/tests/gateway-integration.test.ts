...
const hasApiKey = !!process.env.AI_STATS_API_KEY;
const testIf = (condition: boolean) => (condition ? it : it.skip);

// Only create provider instance if API key is present
const aiStats = hasApiKey ? createAIStats() : null as any;
...
if (!hasApiKey) {
  console.log('\n⚠️  Gateway integration tests skipped (no AI_STATS_API_KEY found)');
  console.log('   Set AI_STATS_API_KEY to run integration tests against the gateway\n');
}
