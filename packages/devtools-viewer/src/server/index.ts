...
// Configuration
const DEVTOOLS_DIR = process.env.AI_STATS_DEVTOOLS_DIR || ".ai-stats-devtools";
const PORT = parseInt(process.env.PORT || "4983", 10);

const writer = new DevToolsWriter(DEVTOOLS_DIR);
...
app.get("/assets/*", async (c) => {
  ...
  const fullPath = path.join(DEVTOOLS_DIR, "assets", assetPath);
...
export function startServer(port: number = PORT) {
  ...
  console.log(`📂 Watching directory: ${path.resolve(DEVTOOLS_DIR)}`);
