...
function renderClient(): string {
	return [
		...
		"\tstd::string request_id() const { auto value = headers.find(\"x-request-id\"); if (value != headers.end()) return value->second; value = headers.find(\"X-Request-ID\"); return value == headers.end() ? std::string{} : value->second; }",
		"\tstd::optional<int> retry_after_seconds() const { auto value = headers.find(\"retry-after\"); if (value == headers.end()) value = headers.find(\"Retry-After\"); if (value == headers.end()) return std::nullopt; try { return std::max(0, std::stoi(value->second)); } catch (...) { return std::nullopt; } }",
		"};",
		"",
		"class Transport {",
		"public:",
