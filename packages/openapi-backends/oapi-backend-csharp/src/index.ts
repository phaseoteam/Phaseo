...
function renderClient(): string {
	return [
		...
		"\tpublic string? RequestId => GetString(\"request_id\") ?? GetHeader(\"x-request-id\");",
		"\tpublic string? GenerationId => GetString(\"generation_id\");",
		"\tpublic string? Code => GetString(\"code\") ?? GetString(\"error\");",
		"\tpublic string? ErrorType => GetString(\"error_type\");",
		"\tpublic string? ErrorOrigin => GetString(\"error_origin\");",
		"\tpublic string? Action => GetString(\"action\");",
		"\tpublic bool? Retryable => GetBool(\"retryable\");",
		"\tpublic long? RetryAfterSeconds => GetLong(\"retry_after_seconds\") ?? GetRetryAfterHeader();",
		"\tpublic string? DocsUrl => GetString(\"docs_url\");",
		"\tpublic string? SupportUrl => GetString(\"support_url\");",
		"\tpublic object? Details => Payload.TryGetValue(\"details\", out var value) ? value : null;",
		"",
		"\tpublic ApiErrorException(int statusCode, string responseBody, string message, Dictionary<string, string>? headers = null, string? statusText = null)",
		"\t\t: base(message)",
		"\t{",
		...
		"\t}",
		"",
		"\tprivate string? GetString(string key) => Payload.TryGetValue(key, out var value) ? value?.ToString() : null;",
		...
		"\t}",
		"\tprivate long? GetLong(string key) => Payload.TryGetValue(key, out var value) && long.TryParse(value?.ToString(), out var number) ? number : null;",
		"\tprivate string? GetHeader(string key) => Headers.TryGetValue(key, out var value) ? value : null;",
		"\tprivate long? GetRetryAfterHeader() => long.TryParse(GetHeader(\"retry-after\"), out var seconds) ? Math.Max(0, seconds) : null;",
		"\tprivate static Dictionary<string, object?> ParsePayload(string responseBody)",
		"\t{",
		...
		"\t\ttry",
		"\t\t{",
		"\t\t\tvar payload = JsonSerializer.Deserialize<Dictionary<string, object?>>(responseBody);",
		"\t\t\tif (payload?.TryGetValue(\"message\", out var value) == true && !string.IsNullOrWhiteSpace(value?.ToString()))",
		"\t\t\t{",
		"\t\t\t\treturn $\"Request failed with status code {statusCode}: {value}\";",
		"\t\t\t}",
		"\t\t}",
		"\t\tcatch (JsonException)",
		"\t\t{",
		...
		"\t}",
		"",
		"\tprivate static ApiErrorException BuildApiErrorException(HttpResponseMessage response, string raw)",
		"\t{",
		...
		"\t\t\theaders[header.Key.ToLowerInvariant()] = string.Join(\",\", header.Value);",
		"\t\t}",
		"\t\treturn new ApiErrorException((int)response.StatusCode, raw, BuildErrorMessage((int)response.StatusCode, raw), headers, response.ReasonPhrase);",
		"}",
		"",
		...
		"\t\t{",
		"\t\t\tthrow BuildApiErrorException(response, raw);",
		"\t\t}",
