...
function renderClient(): string {
	return [
		...
		"",
		"\t\tpublic ApiException(int statusCode, String responseBody, String message, Map<String, String> headers) {",
		"\t\t\tsuper(message);",
		...
		"\t\t\tthis(statusCode, responseBody, message, new HashMap<>());",
		"\t\t}",
		"",
		...
		"\t\t\tObject payloadId = payload.get(\"request_id\");",
		"\t\t\tif (payloadId != null && !payloadId.toString().isBlank()) return payloadId.toString();",
		"\t\t\treturn headers.get(\"x-request-id\");",
		"\t\t}",
		"\t\tpublic String getGenerationId() { return valueAsString(\"generation_id\"); }",
		"\t\tpublic String getCode() { String code = valueAsString(\"code\"); return code == null ? valueAsString(\"error\") : code; }",
		"\t\tpublic String getErrorType() { return valueAsString(\"error_type\"); }",
		"\t\tpublic String getErrorOrigin() { return valueAsString(\"error_origin\"); }",
		"\t\tpublic String getAction() { return valueAsString(\"action\"); }",
		"\t\tpublic String getDocsUrl() { return valueAsString(\"docs_url\"); }",
		"\t\tpublic String getSupportUrl() { return valueAsString(\"support_url\"); }",
		"\t\tpublic Object getDetails() { return payload.get(\"details\"); }",
		"\t\tpublic Boolean getRetryable() { Object value = payload.get(\"retryable\"); return value instanceof Boolean ? (Boolean) value : null; }",
		"\t\tpublic Integer getRetryAfterSeconds() {",
		"\t\t\tObject value = payload.get(\"retry_after_seconds\");",
		"\t\t\tif (value instanceof Number) return Math.max(0, ((Number) value).intValue());",
		"\t\t\ttry { return headers.containsKey(\"retry-after\") ? Math.max(0, Integer.parseInt(headers.get(\"retry-after\"))) : null; } catch (NumberFormatException ignored) { return null; }",
		"\t\t}",
		"\t\tprivate String valueAsString(String key) { Object value = payload.get(key); return value == null ? null : value.toString(); }",
		"\t\tprivate static Map<String, Object> parsePayload(String body) {",
		"\t\t\tif (body == null || body.isBlank()) return new HashMap<>();",
		"\t\t\ttry { return new ObjectMapper().readValue(body, new TypeReference<Map<String, Object>>() {}); } catch (Exception ignored) { return new HashMap<>(); }",
		"\t\t}",
		"",
		...
		"\t\tMap<String, Object> payload = ApiException.parsePayload(responseBody);",
		"\t\tObject message = payload.get(\"message\");",
		"\t\tif (message != null && !message.toString().isBlank()) return \"Request failed: \" + statusCode + \": \" + message;",
		...
		"\t\t\tif (!entry.getValue().isEmpty()) headers.put(entry.getKey().toLowerCase(), entry.getValue().get(0));",
		"\t\t}",
		"\t\treturn headers;",
		"\t}",
		"",
		...
		"\t\t\tthrow new ApiException(response.statusCode(), response.body(), buildErrorMessage(response.statusCode(), response.body()), responseHeaders(response));",
		"\t\t}",
		...
		"\t\t\t\tthrow new ApiException(response.statusCode(), raw, buildErrorMessage(response.statusCode(), raw), responseHeaders(response));",
		"\t\t\t}",
		"\t\t}",
		"\t\treturn response.body();",
		"\t}",
