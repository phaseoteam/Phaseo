...
function renderClient(): string {
	return [
		...
		"\tif r == nil { return \"\" }",
		"\tif value := r.Headers.Get(\"X-Request-Id\"); value != \"\" { return value }",
		"\treturn r.Headers.Get(\"X-Phaseo-Request-Id\")",
		"}",
		"",
		"func (r *Response) TraceURL() string {",
		"\tif id := r.RequestID(); id != \"\" { return \"https://phaseo.app/settings/usage/logs/requests/\" + url.PathEscape(id) }",
		"\treturn \"\"",
		"}",
		"",
		"type Client struct {",
		...
		"\tif e == nil { return \"\" }",
		"\tif value := e.Headers.Get(\"X-Request-Id\"); value != \"\" { return value }",
		"\treturn e.Headers.Get(\"X-Phaseo-Request-Id\")",
		"}",
		"",
		"func (e *HTTPError) TraceURL() string {",
		"\tif id := e.RequestID(); id != \"\" { return \"https://phaseo.app/settings/usage/logs/requests/\" + url.PathEscape(id) }",
		"\treturn \"\"",
		"}",
		"",
		...
		"\tif options == nil { options = &RequestOptions{} }",
		...
		"\tif timeout < 0 { return nil, fmt.Errorf(\"timeout must be non-negative\") }",
		"\tif timeout > 0 {",
		...
		"\tif options.MaxRetries != nil { retries = *options.MaxRetries }",
		"\tif retries < 0 || retries > 10 { return nil, fmt.Errorf(\"max retries must be between 0 and 10\") }",
		"\tif method != http.MethodGet && method != http.MethodHead { retries = 0 }",
		"\tstarted := time.Now()",
		"\tfor attempt := 0; ; attempt++ {",
		"\t\treq, err := http.NewRequestWithContext(ctx, method, endpoint, bytes.NewReader(bodyBytes))",
		"\t\tif err != nil { return nil, err }",
		"\t\tfor key, value := range c.Headers { req.Header.Set(key, value) }",
		"\t\tfor key, value := range options.Headers { req.Header.Set(key, value) }",
		"\t\tif options.IdempotencyKey != \"\" { req.Header.Set(\"Idempotency-Key\", options.IdempotencyKey) }",
		...
		"\t\tif c.OnRequest != nil { c.OnRequest(event) }",
		"\t\tresp, err := c.HTTPClient.Do(req)",
		"\t\tif err != nil {",
		"\t\t\tif attempt >= retries { return nil, err }",
		"\t\t\tdelay := backoff(attempt)",
		"\t\t\tif c.OnRetry != nil { c.OnRetry(RetryEvent{RequestEvent: event, Delay: delay, Err: err}) }",
		...
		"\t\t\tif c.OnRetry != nil { c.OnRetry(RetryEvent{RequestEvent: event, StatusCode: resp.StatusCode, Delay: delay}) }",
		"\t\t\tif err := wait(ctx, delay); err != nil { return nil, err }",
		"\t\t\tcontinue",
		"\t\t}",
		"\t\tif c.OnResponse != nil { c.OnResponse(ResponseEvent{RequestEvent: event, StatusCode: resp.StatusCode, RequestID: response.RequestID(), Elapsed: time.Since(started)}) }",
		...
		"\tif json.Unmarshal(data, &body) != nil { return \"\" }",
		"\tif value, ok := body[\"code\"].(string); ok { return value }",
		"\tif value, ok := body[\"error\"].(string); ok { return value }",
		"\tif value, ok := body[\"error\"].(map[string]any); ok { if code, ok := value[\"code\"].(string); ok { return code } }",
		"\treturn \"\"",
		"}",
		"",
		"func retryableStatus(status int) bool { return status == 408 || status == 429 || status == 500 || status == 502 || status == 503 || status == 504 }",
		"func backoff(attempt int) time.Duration { delay := 250 * time.Millisecond * time.Duration(1<<attempt); if delay > 5*time.Second { return 5*time.Second }; return delay }",
		"func retryDelay(value string, attempt int) time.Duration {",
		"\tif seconds, err := strconv.ParseFloat(value, 64); err == nil && seconds >= 0 { return time.Duration(seconds * float64(time.Second)) }",
		"\tif at, err := http.ParseTime(value); err == nil { if delay := time.Until(at); delay > 0 { return delay }; return 0 }",
