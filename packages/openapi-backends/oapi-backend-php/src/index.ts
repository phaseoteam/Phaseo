...
function renderClient(): string {
	return [
		...
		"",
		"\tpublic function __construct(int $statusCode, string $responseBody, ?string $message = null, array $headers = [])",
		"\t{",
		...
		"\tpublic function getRequestId(): ?string { $value = $this->payload[\"request_id\"] ?? $this->headers[\"x-request-id\"] ?? null; return is_string($value) && trim($value) !== \"\" ? trim($value) : null; }",
		"\tpublic function getGenerationId(): ?string { return $this->stringValue(\"generation_id\"); }",
		"\tpublic function getErrorCode(): ?string { return $this->stringValue(\"code\") ?? $this->stringValue(\"error\"); }",
		"\tpublic function getErrorType(): ?string { return $this->stringValue(\"error_type\"); }",
		"\tpublic function getErrorOrigin(): ?string { return $this->stringValue(\"error_origin\"); }",
		"\tpublic function getAction(): ?string { return $this->stringValue(\"action\"); }",
		"\tpublic function getDocsUrl(): ?string { return $this->stringValue(\"docs_url\"); }",
		"\tpublic function getSupportUrl(): ?string { return $this->stringValue(\"support_url\"); }",
		"\tpublic function getDetails(): mixed { return $this->payload[\"details\"] ?? null; }",
		"\tpublic function getRetryable(): ?bool { return is_bool($this->payload[\"retryable\"] ?? null) ? $this->payload[\"retryable\"] : null; }",
		"\tpublic function getRetryAfterSeconds(): ?int { $value = $this->payload[\"retry_after_seconds\"] ?? $this->headers[\"retry-after\"] ?? null; return is_numeric($value) ? max(0, (int) $value) : null; }",
		"\tprivate function stringValue(string $key): ?string { $value = $this->payload[$key] ?? null; return is_string($value) && trim($value) !== \"\" ? trim($value) : null; }",
		"}",
		"",
		"class Client",
		"{",
		...
		"\t\t}",
		"\t\tif ($status >= 400) {",
		"\t\t\tthrow new RequestException($status, (string) $response, null, $responseHeaders);",
		"\t\t}",
		...
		"\t\t\t\tthrow new RequestException((int) $match[1], $raw === false ? \"\" : $raw, null, $responseHeaders);",
		"\t\t\t}",
