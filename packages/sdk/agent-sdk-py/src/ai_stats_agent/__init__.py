...
class GatewayAgentClient:
    def __init__(
        ...
        *,
        ...
        if client is None:
            ...
            api_key = options.get("api_key") or os.environ.get("AI_STATS_API_KEY")
            if not api_key:
                raise ValueError("AI_STATS_API_KEY is required")
            base_url = options.get("base_url") or os.environ.get("AI_STATS_BASE_URL")
            client = AIStats(api_key=api_key, base_url=base_url) if base_url else AIStats(api_key=api_key)
        self._client = client
        self._model = model
        self._preset = preset
    ...
    def generate(self, request: AgentModelRequest) -> AgentModelResponse:
        payload: Dict[str, Any] = {
            "model": request.model or self._model or _to_preset_alias(self._preset) or "ai-stats/free",
