...
```php
$report = $client->checkModelParameters("openai/gpt-5.4", ["temperature" => 0.7], ["endpoint" => "responses"]);
$preflight = $client->preflightRequest(["model" => "openai/gpt-5.4", "input" => "Hello", "temperature" => 0.7], ["endpoint" => "responses"]);
```

`PageIterator` provides lazy offset pagination, and `JobHandle` refreshes or waits
for an existing job ID without resubmitting it.
