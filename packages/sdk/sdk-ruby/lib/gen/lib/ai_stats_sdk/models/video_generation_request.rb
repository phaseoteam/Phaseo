=begin
#AI Stats Gateway API

#A gateway API for accessing various AI models with OpenAI-compatible endpoints.

The version of the OpenAPI document: 1.0.0

Generated by: https://openapi-generator.tech
Generator version: 7.17.0

=end

require 'date'
require 'time'

module AIStatsSdk
  class VideoGenerationRequest < ApiModelBase
    attr_accessor :model

    attr_accessor :prompt

    attr_accessor :seconds

    attr_accessor :size

    attr_accessor :input_reference

    attr_accessor :input_reference_mime_type

    attr_accessor :duration

    attr_accessor :duration_seconds

    attr_accessor :ratio

    attr_accessor :aspect_ratio

    attr_accessor :resolution

    attr_accessor :negative_prompt

    attr_accessor :sample_count

    attr_accessor :seed

    attr_accessor :person_generation

    attr_accessor :output_storage_uri

    attr_accessor :provider

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'model' => :'model',
        :'prompt' => :'prompt',
        :'seconds' => :'seconds',
        :'size' => :'size',
        :'input_reference' => :'input_reference',
        :'input_reference_mime_type' => :'input_reference_mime_type',
        :'duration' => :'duration',
        :'duration_seconds' => :'duration_seconds',
        :'ratio' => :'ratio',
        :'aspect_ratio' => :'aspect_ratio',
        :'resolution' => :'resolution',
        :'negative_prompt' => :'negative_prompt',
        :'sample_count' => :'sample_count',
        :'seed' => :'seed',
        :'person_generation' => :'person_generation',
        :'output_storage_uri' => :'output_storage_uri',
        :'provider' => :'provider'
      }
    end

    # Returns attribute mapping this model knows about
    def self.acceptable_attribute_map
      attribute_map
    end

    # Returns all the JSON keys this model knows about
    def self.acceptable_attributes
      acceptable_attribute_map.values
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'model' => :'String',
        :'prompt' => :'String',
        :'seconds' => :'VideoGenerationRequestSeconds',
        :'size' => :'String',
        :'input_reference' => :'String',
        :'input_reference_mime_type' => :'String',
        :'duration' => :'Integer',
        :'duration_seconds' => :'Integer',
        :'ratio' => :'String',
        :'aspect_ratio' => :'String',
        :'resolution' => :'String',
        :'negative_prompt' => :'String',
        :'sample_count' => :'Integer',
        :'seed' => :'Integer',
        :'person_generation' => :'String',
        :'output_storage_uri' => :'String',
        :'provider' => :'ProviderRoutingOptions'
      }
    end

    # List of attributes with nullable: true
    def self.openapi_nullable
      Set.new([
      ])
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AIStatsSdk::VideoGenerationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AIStatsSdk::VideoGenerationRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'model')
        self.model = attributes[:'model']
      else
        self.model = nil
      end

      if attributes.key?(:'prompt')
        self.prompt = attributes[:'prompt']
      else
        self.prompt = nil
      end

      if attributes.key?(:'seconds')
        self.seconds = attributes[:'seconds']
      end

      if attributes.key?(:'size')
        self.size = attributes[:'size']
      end

      if attributes.key?(:'input_reference')
        self.input_reference = attributes[:'input_reference']
      end

      if attributes.key?(:'input_reference_mime_type')
        self.input_reference_mime_type = attributes[:'input_reference_mime_type']
      end

      if attributes.key?(:'duration')
        self.duration = attributes[:'duration']
      end

      if attributes.key?(:'duration_seconds')
        self.duration_seconds = attributes[:'duration_seconds']
      end

      if attributes.key?(:'ratio')
        self.ratio = attributes[:'ratio']
      end

      if attributes.key?(:'aspect_ratio')
        self.aspect_ratio = attributes[:'aspect_ratio']
      end

      if attributes.key?(:'resolution')
        self.resolution = attributes[:'resolution']
      end

      if attributes.key?(:'negative_prompt')
        self.negative_prompt = attributes[:'negative_prompt']
      end

      if attributes.key?(:'sample_count')
        self.sample_count = attributes[:'sample_count']
      end

      if attributes.key?(:'seed')
        self.seed = attributes[:'seed']
      end

      if attributes.key?(:'person_generation')
        self.person_generation = attributes[:'person_generation']
      end

      if attributes.key?(:'output_storage_uri')
        self.output_storage_uri = attributes[:'output_storage_uri']
      end

      if attributes.key?(:'provider')
        self.provider = attributes[:'provider']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      warn '[DEPRECATED] the `list_invalid_properties` method is obsolete'
      invalid_properties = Array.new
      if @model.nil?
        invalid_properties.push('invalid value for "model", model cannot be nil.')
      end

      if @prompt.nil?
        invalid_properties.push('invalid value for "prompt", prompt cannot be nil.')
      end

      if !@duration.nil? && @duration > 120
        invalid_properties.push('invalid value for "duration", must be smaller than or equal to 120.')
      end

      if !@duration.nil? && @duration < 1
        invalid_properties.push('invalid value for "duration", must be greater than or equal to 1.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      warn '[DEPRECATED] the `valid?` method is obsolete'
      return false if @model.nil?
      return false if @prompt.nil?
      return false if !@duration.nil? && @duration > 120
      return false if !@duration.nil? && @duration < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] model Value to be assigned
    def model=(model)
      if model.nil?
        fail ArgumentError, 'model cannot be nil'
      end

      @model = model
    end

    # Custom attribute writer method with validation
    # @param [Object] prompt Value to be assigned
    def prompt=(prompt)
      if prompt.nil?
        fail ArgumentError, 'prompt cannot be nil'
      end

      @prompt = prompt
    end

    # Custom attribute writer method with validation
    # @param [Object] duration Value to be assigned
    def duration=(duration)
      if duration.nil?
        fail ArgumentError, 'duration cannot be nil'
      end

      if duration > 120
        fail ArgumentError, 'invalid value for "duration", must be smaller than or equal to 120.'
      end

      if duration < 1
        fail ArgumentError, 'invalid value for "duration", must be greater than or equal to 1.'
      end

      @duration = duration
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          model == o.model &&
          prompt == o.prompt &&
          seconds == o.seconds &&
          size == o.size &&
          input_reference == o.input_reference &&
          input_reference_mime_type == o.input_reference_mime_type &&
          duration == o.duration &&
          duration_seconds == o.duration_seconds &&
          ratio == o.ratio &&
          aspect_ratio == o.aspect_ratio &&
          resolution == o.resolution &&
          negative_prompt == o.negative_prompt &&
          sample_count == o.sample_count &&
          seed == o.seed &&
          person_generation == o.person_generation &&
          output_storage_uri == o.output_storage_uri &&
          provider == o.provider
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [model, prompt, seconds, size, input_reference, input_reference_mime_type, duration, duration_seconds, ratio, aspect_ratio, resolution, negative_prompt, sample_count, seed, person_generation, output_storage_uri, provider].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      attributes = attributes.transform_keys(&:to_sym)
      transformed_hash = {}
      openapi_types.each_pair do |key, type|
        if attributes.key?(attribute_map[key]) && attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = nil
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[attribute_map[key]].is_a?(Array)
            transformed_hash["#{key}"] = attributes[attribute_map[key]].map { |v| _deserialize($1, v) }
          end
        elsif !attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = _deserialize(type, attributes[attribute_map[key]])
        end
      end
      new(transformed_hash)
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = self.class.openapi_nullable.include?(attr)
          next if !is_nullable || (is_nullable && !instance_variable_defined?(:"@#{attr}"))
        end

        hash[param] = _to_hash(value)
      end
      hash
    end

  end

end
