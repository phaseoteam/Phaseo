...
module AIStatsAgentSdk
  ...
  class GatewayAgentClient
    def initialize(client: nil, api_key: nil, base_url: nil, model: nil, preset: nil, provider: nil, reasoning: nil,
      ...
      @client = client || AIStatsSdk::AIStats.new(
        api_key: api_key,
        base_path: base_url || ENV["AI_STATS_BASE_URL"] || "https://api.phaseo.app/v1"
      )
      @model = model
      @preset = preset
      @provider = provider
    ...
    def generate(request)
      payload = {
        model: request.model || @model || preset_alias(@preset) || "ai-stats/free",
