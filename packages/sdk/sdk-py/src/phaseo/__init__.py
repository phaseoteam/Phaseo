...
PhaseoLogLevel: TypeAlias = Literal["info", "warn", "error"]
PhaseoLogger: TypeAlias = Callable[[PhaseoLogLevel, str, dict[str, Any]], None]
...
def _build_lifecycle_message(
    ...
    if status == "retired":
        if retirement_date:
            return f'[phaseo] Model "{model_id}" is retired as of {retirement_date}.{replacement}'
        return f'[phaseo] Model "{model_id}" is retired.{replacement}'
    if status == "deprecated":
        if retirement_date:
            return (
                f'[phaseo] Model "{model_id}" is deprecated and scheduled for retirement on {retirement_date}.'
                f"{replacement}"
            )
        if deprecation_date:
            return f'[phaseo] Model "{model_id}" has been deprecated since {deprecation_date}.{replacement}'
        return f'[phaseo] Model "{model_id}" is deprecated.{replacement}'
    return None
...
def _build_inactive_model_request_message(info: ModelLifecycleInfo) -> str:
    ...
    return (
        f'[phaseo] Model "{info.get("model_id")}" is not active for inference '
...
__all__ = [
    ...
    "ModelId",
    "ResponseStreamChunk",
    "compute_async_webhook_signature",
    "create_phaseo_devtools",
    "verify_async_webhook_signature",
    "models",
    "ops",
]
