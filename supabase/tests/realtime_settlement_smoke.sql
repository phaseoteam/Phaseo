...
begin
  ...
  where w.workspace_id = '6108396e-0e12-425d-91ff-a02d39a346e0' and coalesce(w.balance_nanos, 0) - coalesce(w.reserved_nanos, 0) > 1
  ...
  update public.gateway_realtime_sessions set status = 'connected', connected_at = now() where session_id = v_session_id;
  perform * from public.gateway_realtime_extend_hold_once(v_workspace_id, v_session_id, v_reservation_prefix || '0002', 3, 1);
  perform * from public.gateway_realtime_extend_hold_once(v_workspace_id, v_session_id, v_reservation_prefix || '0002', 3, 1);
  if (select reserved_nanos from public.wallets where workspace_id = v_workspace_id) <> v_before_reserved + 3 then
    raise exception 'realtime_smoke_extension_not_idempotent';
  end if;
  ...
  if v_request_status <> 499 or v_request_cost <> 2 or v_request_success then
    raise exception 'realtime_smoke_request_not_settled_atomically';
  end if;

  perform * from public.gateway_realtime_settle_once(p_workspace_id => v_workspace_id, p_session_id => v_session_id, p_final_cost_nanos => 2, p_status => 'cancelled');
  if (select balance_nanos from public.wallets where workspace_id = v_workspace_id) <> v_before_balance - 2 then
    raise exception 'realtime_smoke_double_charge';
  end if;
  if not exists (select 1 from public.gateway_realtime_sessions where session_id = v_session_id and reserved_nanos = 3 and captured_nanos = 2 and released_nanos = 1) then
    raise exception 'realtime_smoke_settlement_totals';
  end if;
