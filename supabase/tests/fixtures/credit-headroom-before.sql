-- Captured existing production function definitions on 2026-09-16; local tests only.
create role anon;
create role authenticated;
create role service_role;
create table public.wallets (
 workspace_id uuid primary key, balance_nanos bigint not null, reserved_nanos bigint not null default 0,
 low_balance_threshold bigint default 0, auto_top_up_amount bigint default 0,
 auto_top_up_enabled boolean default false, auto_top_up_account_id text, stripe_customer_id text
);
create table public.gateway_request_charges (
 workspace_id uuid, request_id text, cost_nanos bigint, status text,
 created_at timestamptz, updated_at timestamptz, error_message text,
 deducted_status text, auto_top_up_required boolean,
 primary key(workspace_id,request_id)
);
grant select on public.wallets to service_role;
CREATE OR REPLACE FUNCTION public.deduct_and_check_top_up(p_workspace_id uuid, p_cost_nanos bigint)
 RETURNS json
 LANGUAGE plpgsql
...
$function$
;
CREATE OR REPLACE FUNCTION public.gateway_deduct_and_check_top_up_once(p_workspace_id uuid, p_request_id text, p_cost_nanos bigint)
 RETURNS TABLE(applied boolean, already_applied boolean, status text, auto_top_up_amount_nanos bigint, auto_top_up_account_id text, stripe_customer_id text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
$function$
;
revoke all on function public.gateway_deduct_and_check_top_up_once(uuid,text,bigint) from public, anon, authenticated;
