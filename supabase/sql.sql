...
CREATE TABLE public.users (
  ...
  CONSTRAINT users_display_time_zone_check CHECK (char_length(display_time_zone) >= 1 AND char_length(display_time_zone) <= 100 AND (display_time_zone = ANY (ARRAY['system'::text, 'UTC'::text]) OR display_time_zone ~ '^[A-Za-z0-9._+-]+(/[A-Za-z0-9._+-]+)+$'::text)),
