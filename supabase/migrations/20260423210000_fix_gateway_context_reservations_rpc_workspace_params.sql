-- Fix RPC parameter names/body for gateway context wrapper after team->workspace
-- migration. PostgREST matches RPC JSON keys to argument names, so this function
-- must expose workspace_id (not team_id).

drop function if exists public.gateway_fetch_request_context_with_reservations(uuid, text, text, uuid);

create function public.gateway_fetch_request_context_with_reservations(
  workspace_id uuid,
  model text,
  endpoint text,
  api_key_id uuid
