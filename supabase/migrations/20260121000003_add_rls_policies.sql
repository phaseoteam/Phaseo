...
create policy wallets_select_own_team
  ...
  to authenticated
  using (public.is_workspace_member(workspace_id));
-- workspace_members: allow members to view team membership rows for teams they belong to
drop policy if exists workspace_members_select_own_team on public.workspace_members;
create policy workspace_members_select_own_team
  on public.workspace_members
  for select
  to authenticated
  using (public.is_workspace_member(workspace_id));
-- teams: allow members to view teams they belong to
drop policy if exists teams_select_own_team on public.workspaces;
...
-- workspace_invites: team members can manage invites for their teams
drop policy if exists workspace_invites_select_own_team on public.workspace_invites;
...
create policy workspace_invites_select_own_team
...
create policy workspace_invites_delete_own_team
  ...
  to authenticated
  using (public.is_workspace_member(workspace_id));
-- workspace_join_requests: requester or team members can view; team members can update
...
create policy workspace_join_requests_select
  ...
  for select
  to authenticated
  using (public.is_workspace_member(workspace_id) or requester_user_id = auth.uid());
