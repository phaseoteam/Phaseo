...
set slug = lower(regexp_replace(name, '^@', ''))
where slug is null or btrim(slug) = '';

alter table public.presets
  alter column slug set not null;

create unique index if not exists presets_workspace_id_slug_idx
  on public.presets (workspace_id, slug);

create index if not exists presets_slug_idx
