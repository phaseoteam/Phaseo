-- Preserve service-tier attribution in request facts and expose tier-specific
-- provider performance. Historical rows without a trustworthy tier remain
-- unclassified instead of being incorrectly folded into Standard.

alter table public.v2_request_facts
  add column if not exists service_tier_requested text,
  add column if not exists service_tier_observed text,
  add column if not exists service_tier_slug text;
...
revoke execute on function public.normalize_v2_service_tier(text)
  from public, anon, authenticated;
grant execute on function public.normalize_v2_service_tier(text)
  to service_role;
...
drop trigger if exists v2_request_facts_service_tier on public.v2_request_facts;
create trigger v2_request_facts_service_tier
before insert or update of service_tier_requested, service_tier_observed, service_tier_slug, safe_metadata, endpoint
on public.v2_request_facts
for each row execute function public.set_v2_request_fact_service_tier();

update public.v2_request_facts fact
set
  service_tier_requested = public.normalize_v2_service_tier(fact.safe_metadata->>'service_tier_requested'),
  service_tier_observed = public.normalize_v2_service_tier(fact.safe_metadata->>'service_tier_observed'),
  service_tier_slug = coalesce(
    public.normalize_v2_service_tier(fact.safe_metadata->>'service_tier'),
    public.normalize_v2_service_tier(fact.safe_metadata->>'service_tier_observed'),
    public.normalize_v2_service_tier(fact.safe_metadata->>'service_tier_requested'),
    case when fact.endpoint = 'batch' then 'batch' else null end
  )
where fact.service_tier_slug is null
  and (
    fact.endpoint = 'batch'
    or fact.safe_metadata ? 'service_tier'
    or fact.safe_metadata ? 'service_tier_observed'
    or fact.safe_metadata ? 'service_tier_requested'
  );
...
alter table public.v2_request_facts
  add constraint v2_request_facts_service_tier_requested_check
    check (service_tier_requested is null or service_tier_requested in ('standard', 'priority', 'flex', 'batch')) not valid,
  add constraint v2_request_facts_service_tier_observed_check
    check (service_tier_observed is null or service_tier_observed in ('standard', 'priority', 'flex', 'batch')) not valid,
  add constraint v2_request_facts_service_tier_slug_check
    check (service_tier_slug is null or service_tier_slug in ('standard', 'priority', 'flex', 'batch')) not valid;
alter table public.v2_request_facts
...
create index if not exists v2_request_facts_model_tier_time_idx
  on public.v2_request_facts (
...
comment on column public.v2_request_facts.service_tier_requested is
  'Canonical service tier requested by the client; fast is stored as priority.';
comment on column public.v2_request_facts.service_tier_observed is
  'Canonical service tier reported by the upstream provider, when available.';
comment on column public.v2_request_facts.service_tier_slug is
  'Effective canonical service tier used for pricing and performance attribution.';
...
as $$
  ...
  select
    ...
    '{}'::jsonb, '{}'::jsonb, coalesce(bucketed.buckets, '[]'::jsonb), aggregate.last_request_at
...
comment on function public.get_v2_model_provider_tier_health_metrics(text, integer, numeric) is
  'Provider health and synchronous performance grouped by effective service tier. Batch latency and throughput are intentionally omitted.';
