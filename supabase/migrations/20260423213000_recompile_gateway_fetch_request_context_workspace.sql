-- Recompile gateway context RPC to ensure workspace-era column/table references are active.
-- This overwrites any stale remote body that still references team-era columns such as k.team_id.

drop function if exists public.gateway_fetch_request_context(uuid, text, text, uuid);
