-- Workspace member deletion holds the membership row before its trigger revokes
-- OAuth authorizations. Match that lock order in token issuance and refresh
-- rotation so revocation races serialize instead of deadlocking.
