-- Recompile workspace usage rollup SQL functions after team->workspace rename.
-- Fixes stale function bodies still referencing team-era relation names.

create table if not exists public.gateway_usage_rollup_workspace_request_state (
...
begin
  ...
  do update
  set
    requests = public.gateway_usage_rollup_15m_workspace_provider_model.requests + excluded.requests,
    success_requests = public.gateway_usage_rollup_15m_workspace_provider_model.success_requests + excluded.success_requests,
    total_tokens = public.gateway_usage_rollup_15m_workspace_provider_model.total_tokens + excluded.total_tokens,
    total_cost_nanos = public.gateway_usage_rollup_15m_workspace_provider_model.total_cost_nanos + excluded.total_cost_nanos,
    latency_sum_ms = public.gateway_usage_rollup_15m_workspace_provider_model.latency_sum_ms + excluded.latency_sum_ms,
    latency_samples = public.gateway_usage_rollup_15m_workspace_provider_model.latency_samples + excluded.latency_samples,
    throughput_sum = public.gateway_usage_rollup_15m_workspace_provider_model.throughput_sum + excluded.throughput_sum,
    throughput_samples = public.gateway_usage_rollup_15m_workspace_provider_model.throughput_samples + excluded.throughput_samples;
...
declare
  ...
  v_prev public.gateway_usage_rollup_workspace_request_state%rowtype;
...
begin
  ...
  select *
  into v_prev
  from public.gateway_usage_rollup_workspace_request_state s
  where s.request_row_id = p_request_row_id
  for update;

  if found
    and v_prev.request_created_at = v_request.created_at
    and v_prev.workspace_id = v_request.workspace_id
    ...
    update public.gateway_usage_rollup_workspace_request_state
    set updated_at = now()
  ...
  if found then
    perform public.apply_workspace_usage_rollup_delta(
      v_prev.bucket_15m,
      v_prev.workspace_id,
      v_prev.key_id,
      v_prev.provider,
  ...
  insert into public.gateway_usage_rollup_workspace_request_state (
    request_row_id,
    request_created_at,
    workspace_id,
    bucket_15m,
...
comment on function public.apply_workspace_usage_rollup_delta(
  timestamptz,
  uuid,
  uuid,
  text,
