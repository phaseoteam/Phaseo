-- Preserve time-window billing semantics in the v2 SKU projection and expose
-- them through the model pricing RPC used by the Cloudflare Web API.

update public.v2_pricing_skus sku
set metadata = sku.metadata || jsonb_build_object(
  'billing_timestamp_basis', coalesce(rule.billing_timestamp_basis, 'request_start'),
  'time_windows', coalesce(rule.time_windows, '[]'::jsonb)
)
...
set metadata = meter.metadata || jsonb_build_object(
  'billing_timestamp_basis', coalesce(rule.billing_timestamp_basis, 'request_start'),
  'time_windows', coalesce(rule.time_windows, '[]'::jsonb)
)
from public.data_api_pricing_rules rule
where meter.metadata->>'legacy_rule_id' = rule.rule_id::text;
